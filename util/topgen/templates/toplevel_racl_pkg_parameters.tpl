...
<%page args="register_mapping, window_mapping, range_mapping, policy_names, racl_group, m, if_name"/>\
<% import textwrap %>\
<% import math %>\
...
<% has_ranges = if_name and block.bus_interfaces.racl_range_support.get(if_name) and len(range_mapping) > 0 %>\
<% verbose = False %>\
  /**
