...
#define TEST_MAX_IRQ_PERIPHERAL 21
#endif
...
static const top_darjeeling_no_ibex_plic_target_t kHart = kTopDarjeeling_no_ibexPlicTargetIbex0;
 ...
 * as the ISR.
 */
static volatile top_darjeeling_no_ibex_plic_peripheral_t peripheral_expected;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 0 && 0 < TEST_MAX_IRQ_PERIPHERAL
static volatile dif_ac_range_check_irq_t ac_range_check_irq_expected;
static volatile dif_ac_range_check_irq_t ac_range_check_irq_serviced;
#endif
...
#if TEST_MIN_IRQ_PERIPHERAL <= 16 && 16 < TEST_MAX_IRQ_PERIPHERAL
static volatile dif_racl_ctrl_irq_t racl_ctrl_irq_expected;
static volatile dif_racl_ctrl_irq_t racl_ctrl_irq_serviced;
#endif
...
void ottf_external_isr(uint32_t *exc_info) {
  ...
  top_darjeeling_no_ibex_plic_peripheral_t peripheral = (top_darjeeling_no_ibex_plic_peripheral_t)
      top_darjeeling_no_ibex_plic_interrupt_for_peripheral[plic_irq_id];
  ...
  switch (peripheral) {
    ...
    case kTopDarjeelingNoIbexPlicPeripheralAcRangeCheck: {
      dif_ac_range_check_irq_t irq =
          (dif_ac_range_check_irq_t)(plic_irq_id -
                                     (dif_rv_plic_irq_id_t)
                                         kTopDarjeelingNoIbexPlicIrqIdAcRangeCheckDenyCntReached);
      CHECK(irq == ac_range_check_irq_expected,
            "Incorrect ac_range_check IRQ triggered: exp = %d, obs = %d",
            ac_range_check_irq_expected, irq);
      ac_range_check_irq_serviced = irq;

      dif_ac_range_check_irq_state_snapshot_t snapshot;
      CHECK_DIF_OK(dif_ac_range_check_irq_get_state(&ac_range_check, &snapshot));
      CHECK(snapshot == (dif_ac_range_check_irq_state_snapshot_t)(1 << irq),
      ...
      if (0x1 & (1 << irq)) {
        ...
        CHECK_DIF_OK(dif_ac_range_check_irq_force(&ac_range_check, irq, false));
        ...
        // rest of the test.
        if ((0x0 & (1 << irq))) {
          CHECK_DIF_OK(dif_ac_range_check_irq_set_enabled(&ac_range_check, irq, false));
        }
      } else {
        // We acknowledge event type interrupt.
        CHECK_DIF_OK(dif_ac_range_check_irq_acknowledge(&ac_range_check, irq));
      }
      break;
    }
#endif
...
#if TEST_MIN_IRQ_PERIPHERAL <= 1 && 1 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralAlertHandler: {
      dif_alert_handler_irq_t irq =
          (dif_alert_handler_irq_t)(plic_irq_id -
                                    (dif_rv_plic_irq_id_t)
                                        kTopDarjeelingNoIbexPlicIrqIdAlertHandlerClassa);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 2 && 2 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralAonTimerAon: {
      dif_aon_timer_irq_t irq =
          (dif_aon_timer_irq_t)(plic_irq_id -
                                (dif_rv_plic_irq_id_t)
                                    kTopDarjeelingNoIbexPlicIrqIdAonTimerAonWkupTimerExpired);
    ...
    case kTopDarjeelingNoIbexPlicPeripheralCsrng: {
      dif_csrng_irq_t irq =
          (dif_csrng_irq_t)(plic_irq_id -
                            (dif_rv_plic_irq_id_t)
                                kTopDarjeelingNoIbexPlicIrqIdCsrngCsCmdReqDone);
    ...
    case kTopDarjeelingNoIbexPlicPeripheralDma: {
      dif_dma_irq_t irq =
          (dif_dma_irq_t)(plic_irq_id -
                          (dif_rv_plic_irq_id_t)
                              kTopDarjeelingNoIbexPlicIrqIdDmaDmaDone);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 5 && 5 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralEdn0: {
      dif_edn_irq_t irq =
          (dif_edn_irq_t)(plic_irq_id -
                          (dif_rv_plic_irq_id_t)
                              kTopDarjeelingNoIbexPlicIrqIdEdn0EdnCmdReqDone);
    ...
    case kTopDarjeelingNoIbexPlicPeripheralEdn1: {
      dif_edn_irq_t irq =
          (dif_edn_irq_t)(plic_irq_id -
                          (dif_rv_plic_irq_id_t)
                              kTopDarjeelingNoIbexPlicIrqIdEdn1EdnCmdReqDone);
      CHECK(irq == edn_irq_expected,
...
#if TEST_MIN_IRQ_PERIPHERAL <= 6 && 6 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralEntropySrc: {
      dif_entropy_src_irq_t irq =
          (dif_entropy_src_irq_t)(plic_irq_id -
                                  (dif_rv_plic_irq_id_t)
                                      kTopDarjeelingNoIbexPlicIrqIdEntropySrcEsEntropyValid);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 7 && 7 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralGpio: {
      dif_gpio_irq_t irq =
          (dif_gpio_irq_t)(plic_irq_id -
                           (dif_rv_plic_irq_id_t)
                               kTopDarjeelingNoIbexPlicIrqIdGpioGpio0);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 8 && 8 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralHmac: {
      dif_hmac_irq_t irq =
          (dif_hmac_irq_t)(plic_irq_id -
                           (dif_rv_plic_irq_id_t)
                               kTopDarjeelingNoIbexPlicIrqIdHmacHmacDone);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 9 && 9 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralI2c0: {
      dif_i2c_irq_t irq =
...
#if TEST_MIN_IRQ_PERIPHERAL <= 10 && 10 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralKeymgrDpe: {
      dif_keymgr_dpe_irq_t irq =
          (dif_keymgr_dpe_irq_t)(plic_irq_id -
                                 (dif_rv_plic_irq_id_t)
                                     kTopDarjeelingNoIbexPlicIrqIdKeymgrDpeOpDone);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 11 && 11 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralKmac: {
      dif_kmac_irq_t irq =
          (dif_kmac_irq_t)(plic_irq_id -
                           (dif_rv_plic_irq_id_t)
                               kTopDarjeelingNoIbexPlicIrqIdKmacKmacDone);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralMbx0: {
      dif_mbx_irq_t irq =
          (dif_mbx_irq_t)(plic_irq_id -
                          (dif_rv_plic_irq_id_t)
                              kTopDarjeelingNoIbexPlicIrqIdMbx0MbxReady);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralMbx1: {
      dif_mbx_irq_t irq =
          (dif_mbx_irq_t)(plic_irq_id -
                          (dif_rv_plic_irq_id_t)
                              kTopDarjeelingNoIbexPlicIrqIdMbx1MbxReady);
      CHECK(irq == mbx_irq_expected,
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralMbx2: {
      dif_mbx_irq_t irq =
          (dif_mbx_irq_t)(plic_irq_id -
                          (dif_rv_plic_irq_id_t)
                              kTopDarjeelingNoIbexPlicIrqIdMbx2MbxReady);
      CHECK(irq == mbx_irq_expected,
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralMbx3: {
      dif_mbx_irq_t irq =
          (dif_mbx_irq_t)(plic_irq_id -
                          (dif_rv_plic_irq_id_t)
                              kTopDarjeelingNoIbexPlicIrqIdMbx3MbxReady);
      CHECK(irq == mbx_irq_expected,
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralMbx4: {
      dif_mbx_irq_t irq =
          (dif_mbx_irq_t)(plic_irq_id -
                          (dif_rv_plic_irq_id_t)
                              kTopDarjeelingNoIbexPlicIrqIdMbx4MbxReady);
      CHECK(irq == mbx_irq_expected,
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralMbx5: {
      dif_mbx_irq_t irq =
          (dif_mbx_irq_t)(plic_irq_id -
                          (dif_rv_plic_irq_id_t)
                              kTopDarjeelingNoIbexPlicIrqIdMbx5MbxReady);
      CHECK(irq == mbx_irq_expected,
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralMbx6: {
      dif_mbx_irq_t irq =
          (dif_mbx_irq_t)(plic_irq_id -
                          (dif_rv_plic_irq_id_t)
                              kTopDarjeelingNoIbexPlicIrqIdMbx6MbxReady);
      CHECK(irq == mbx_irq_expected,
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralMbxJtag: {
      dif_mbx_irq_t irq =
          (dif_mbx_irq_t)(plic_irq_id -
                          (dif_rv_plic_irq_id_t)
                              kTopDarjeelingNoIbexPlicIrqIdMbxJtagMbxReady);
      CHECK(irq == mbx_irq_expected,
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralMbxPcie0: {
      dif_mbx_irq_t irq =
          (dif_mbx_irq_t)(plic_irq_id -
                          (dif_rv_plic_irq_id_t)
                              kTopDarjeelingNoIbexPlicIrqIdMbxPcie0MbxReady);
      CHECK(irq == mbx_irq_expected,
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralMbxPcie1: {
      dif_mbx_irq_t irq =
          (dif_mbx_irq_t)(plic_irq_id -
                          (dif_rv_plic_irq_id_t)
                              kTopDarjeelingNoIbexPlicIrqIdMbxPcie1MbxReady);
      CHECK(irq == mbx_irq_expected,
...
#if TEST_MIN_IRQ_PERIPHERAL <= 13 && 13 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralOtbn: {
      dif_otbn_irq_t irq =
...
#if TEST_MIN_IRQ_PERIPHERAL <= 14 && 14 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralOtpCtrl: {
      dif_otp_ctrl_irq_t irq =
          (dif_otp_ctrl_irq_t)(plic_irq_id -
                               (dif_rv_plic_irq_id_t)
                                   kTopDarjeelingNoIbexPlicIrqIdOtpCtrlOtpOperationDone);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 15 && 15 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralPwrmgrAon: {
      dif_pwrmgr_irq_t irq =
          (dif_pwrmgr_irq_t)(plic_irq_id -
                             (dif_rv_plic_irq_id_t)
                                 kTopDarjeelingNoIbexPlicIrqIdPwrmgrAonWakeup);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 16 && 16 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralRaclCtrl: {
      dif_racl_ctrl_irq_t irq =
          (dif_racl_ctrl_irq_t)(plic_irq_id -
                                (dif_rv_plic_irq_id_t)
                                    kTopDarjeelingNoIbexPlicIrqIdRaclCtrlRaclError);
      CHECK(irq == racl_ctrl_irq_expected,
            "Incorrect racl_ctrl IRQ triggered: exp = %d, obs = %d",
            racl_ctrl_irq_expected, irq);
      racl_ctrl_irq_serviced = irq;

      dif_racl_ctrl_irq_state_snapshot_t snapshot;
      CHECK_DIF_OK(dif_racl_ctrl_irq_get_state(&racl_ctrl, &snapshot));
      CHECK(snapshot == (dif_racl_ctrl_irq_state_snapshot_t)(1 << irq),
      ...
      if (0x1 & (1 << irq)) {
        // We do not acknowledge status type interrupt at the IP side, but we
        // need to clear the test force register.
        CHECK_DIF_OK(dif_racl_ctrl_irq_force(&racl_ctrl, irq, false));
        // In case this status interrupt is asserted by default, we also
        // disable it at this point so that it does not interfere with the
        // rest of the test.
        if ((0x0 & (1 << irq))) {
          CHECK_DIF_OK(dif_racl_ctrl_irq_set_enabled(&racl_ctrl, irq, false));
        }
      } else {
        // We acknowledge event type interrupt.
        CHECK_DIF_OK(dif_racl_ctrl_irq_acknowledge(&racl_ctrl, irq));
      }
      break;
    }
#endif
...
#if TEST_MIN_IRQ_PERIPHERAL <= 17 && 17 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralSocProxy: {
      dif_soc_proxy_irq_t irq =
          (dif_soc_proxy_irq_t)(plic_irq_id -
                                (dif_rv_plic_irq_id_t)
                                    kTopDarjeelingNoIbexPlicIrqIdSocProxyExternal0);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 18 && 18 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralSpiDevice: {
...
#if TEST_MIN_IRQ_PERIPHERAL <= 19 && 19 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralSpiHost0: {
      dif_spi_host_irq_t irq =
          (dif_spi_host_irq_t)(plic_irq_id -
                               (dif_rv_plic_irq_id_t)
                                   kTopDarjeelingNoIbexPlicIrqIdSpiHost0Error);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 20 && 20 < TEST_MAX_IRQ_PERIPHERAL
    case kTopDarjeelingNoIbexPlicPeripheralUart0: {
      dif_uart_irq_t irq =
          (dif_uart_irq_t)(plic_irq_id -
                           (dif_rv_plic_irq_id_t)
                               kTopDarjeelingNoIbexPlicIrqIdUart0TxWatermark);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 1 && 1 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_ALERT_HANDLER_BASE_ADDR);
  CHECK_DIF_OK(dif_alert_handler_init(base_addr, &alert_handler));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 2 && 2 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_AON_TIMER_AON_BASE_ADDR);
  CHECK_DIF_OK(dif_aon_timer_init(base_addr, &aon_timer_aon));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 3 && 3 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_CSRNG_BASE_ADDR);
  CHECK_DIF_OK(dif_csrng_init(base_addr, &csrng));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 4 && 4 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_DMA_BASE_ADDR);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 5 && 5 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_EDN1_BASE_ADDR);
  CHECK_DIF_OK(dif_edn_init(base_addr, &edn1));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 6 && 6 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_ENTROPY_SRC_BASE_ADDR);
  CHECK_DIF_OK(dif_entropy_src_init(base_addr, &entropy_src));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 7 && 7 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_GPIO_BASE_ADDR);
  CHECK_DIF_OK(dif_gpio_init(base_addr, &gpio));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 8 && 8 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_HMAC_BASE_ADDR);
  CHECK_DIF_OK(dif_hmac_init(base_addr, &hmac));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 9 && 9 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_I2C0_BASE_ADDR);
  CHECK_DIF_OK(dif_i2c_init(base_addr, &i2c0));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 10 && 10 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_KEYMGR_DPE_BASE_ADDR);
  CHECK_DIF_OK(dif_keymgr_dpe_init(base_addr, &keymgr_dpe));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 11 && 11 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_KMAC_BASE_ADDR);
  CHECK_DIF_OK(dif_kmac_init(base_addr, &kmac));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_MBX0_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_mbx_init(base_addr, &mbx0));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_MBX1_CORE_BASE_ADDR);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_MBX3_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_mbx_init(base_addr, &mbx3));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_MBX4_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_mbx_init(base_addr, &mbx4));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_MBX5_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_mbx_init(base_addr, &mbx5));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_MBX6_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_mbx_init(base_addr, &mbx6));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_MBX_JTAG_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_mbx_init(base_addr, &mbx_jtag));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_MBX_PCIE0_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_mbx_init(base_addr, &mbx_pcie0));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_MBX_PCIE1_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_mbx_init(base_addr, &mbx_pcie1));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 13 && 13 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_OTBN_BASE_ADDR);
  CHECK_DIF_OK(dif_otbn_init(base_addr, &otbn));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 14 && 14 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_OTP_CTRL_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_otp_ctrl_init(base_addr, &otp_ctrl));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 15 && 15 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_PWRMGR_AON_BASE_ADDR);
  CHECK_DIF_OK(dif_pwrmgr_init(base_addr, &pwrmgr_aon));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 16 && 16 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_SOC_MBX_RACL_CTRL_BASE_ADDR);
  CHECK_DIF_OK(dif_racl_ctrl_init(base_addr, &racl_ctrl));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 17 && 17 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_SOC_PROXY_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_soc_proxy_init(base_addr, &soc_proxy));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 18 && 18 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_SPI_DEVICE_BASE_ADDR);
  CHECK_DIF_OK(dif_spi_device_init(base_addr, &spi_device));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 19 && 19 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_SPI_HOST0_BASE_ADDR);
  CHECK_DIF_OK(dif_spi_host_init(base_addr, &spi_host0));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 20 && 20 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_UART0_BASE_ADDR);
  CHECK_DIF_OK(dif_uart_init(base_addr, &uart0));
#endif

  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_RV_PLIC_BASE_ADDR);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 0 && 0 < TEST_MAX_IRQ_PERIPHERAL
  CHECK_DIF_OK(dif_ac_range_check_irq_acknowledge_all(&ac_range_check));
#endif
...
#if TEST_MIN_IRQ_PERIPHERAL <= 16 && 16 < TEST_MAX_IRQ_PERIPHERAL
  CHECK_DIF_OK(dif_racl_ctrl_irq_acknowledge_all(&racl_ctrl));
#endif
...
#if TEST_MIN_IRQ_PERIPHERAL <= 0 && 0 < TEST_MAX_IRQ_PERIPHERAL
  dif_ac_range_check_irq_state_snapshot_t ac_range_check_irqs =
      (dif_ac_range_check_irq_state_snapshot_t)0xffffffff;
#endif
...
#if TEST_MIN_IRQ_PERIPHERAL <= 16 && 16 < TEST_MAX_IRQ_PERIPHERAL
  dif_racl_ctrl_irq_state_snapshot_t racl_ctrl_irqs =
      (dif_racl_ctrl_irq_state_snapshot_t)0xffffffff;
#endif
...
#if TEST_MIN_IRQ_PERIPHERAL <= 0 && 0 < TEST_MAX_IRQ_PERIPHERAL
  CHECK_DIF_OK(dif_ac_range_check_irq_restore_all(&ac_range_check, &ac_range_check_irqs));
#endif
...
#if TEST_MIN_IRQ_PERIPHERAL <= 16 && 16 < TEST_MAX_IRQ_PERIPHERAL
  CHECK_DIF_OK(dif_racl_ctrl_irq_restore_all(&racl_ctrl, &racl_ctrl_irqs));
#endif
...
static void peripheral_irqs_trigger(void) {
  ...
  for (dif_ac_range_check_irq_t irq = kDifAcRangeCheckIrqDenyCntReached; irq <= kDifAcRangeCheckIrqDenyCntReached;
       ++irq) {
    ac_range_check_irq_expected = irq;
    LOG_INFO("Triggering ac_range_check IRQ %d.", irq);
    CHECK_DIF_OK(dif_ac_range_check_irq_force(&ac_range_check, irq, true));
    ...
    if ((status_default_mask & 0x1)) {
      CHECK_DIF_OK(dif_ac_range_check_irq_set_enabled(&ac_range_check, irq, true));
    }
    status_default_mask >>= 1;

    // This avoids a race where *irq_serviced is read before
    // entering the ISR.
    IBEX_SPIN_FOR(ac_range_check_irq_serviced == irq, 1);
    LOG_INFO("IRQ %d from ac_range_check is serviced.", irq);
  }
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 1 && 1 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralAlertHandler;
  ...
  // non-DV setups.
  if (kDeviceType == kDeviceSimDV) {
    peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralAonTimerAon;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 3 && 3 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralCsrng;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 6 && 6 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralEntropySrc;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 7 && 7 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralGpio;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 8 && 8 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralHmac;
  status_default_mask = 0x0;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 9 && 9 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralI2c0;
  status_default_mask = 0x0;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 10 && 10 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralKeymgrDpe;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 11 && 11 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralKmac;
  status_default_mask = 0x0;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralMbx0;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralMbx1;
  for (dif_mbx_irq_t irq = kDifMbxIrqMbxReady; irq <= kDifMbxIrqMbxError;
       ++irq) {
    mbx_irq_expected = irq;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralMbx2;
  for (dif_mbx_irq_t irq = kDifMbxIrqMbxReady; irq <= kDifMbxIrqMbxError;
       ++irq) {
    mbx_irq_expected = irq;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralMbx3;
  for (dif_mbx_irq_t irq = kDifMbxIrqMbxReady; irq <= kDifMbxIrqMbxError;
       ++irq) {
    mbx_irq_expected = irq;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralMbx4;
  for (dif_mbx_irq_t irq = kDifMbxIrqMbxReady; irq <= kDifMbxIrqMbxError;
       ++irq) {
    mbx_irq_expected = irq;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralMbx5;
  for (dif_mbx_irq_t irq = kDifMbxIrqMbxReady; irq <= kDifMbxIrqMbxError;
       ++irq) {
    mbx_irq_expected = irq;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralMbx6;
  for (dif_mbx_irq_t irq = kDifMbxIrqMbxReady; irq <= kDifMbxIrqMbxError;
       ++irq) {
    mbx_irq_expected = irq;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralMbxJtag;
  for (dif_mbx_irq_t irq = kDifMbxIrqMbxReady; irq <= kDifMbxIrqMbxError;
       ++irq) {
    mbx_irq_expected = irq;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralMbxPcie0;
  for (dif_mbx_irq_t irq = kDifMbxIrqMbxReady; irq <= kDifMbxIrqMbxError;
       ++irq) {
    mbx_irq_expected = irq;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralMbxPcie1;
  for (dif_mbx_irq_t irq = kDifMbxIrqMbxReady; irq <= kDifMbxIrqMbxError;
       ++irq) {
    mbx_irq_expected = irq;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 13 && 13 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralOtbn;
  ...
  if (kBootStage != kBootStageOwner) {
    peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralOtpCtrl;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 15 && 15 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralPwrmgrAon;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 16 && 16 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralRaclCtrl;
  status_default_mask = 0x0;
  for (dif_racl_ctrl_irq_t irq = kDifRaclCtrlIrqRaclError; irq <= kDifRaclCtrlIrqRaclError;
       ++irq) {
    racl_ctrl_irq_expected = irq;
    LOG_INFO("Triggering racl_ctrl IRQ %d.", irq);
    CHECK_DIF_OK(dif_racl_ctrl_irq_force(&racl_ctrl, irq, true));

    // In this case, the interrupt has not been enabled yet because that would
    // interfere with testing other interrupts. We enable it here and let the
    // interrupt handler disable it again.
    if ((status_default_mask & 0x1)) {
      CHECK_DIF_OK(dif_racl_ctrl_irq_set_enabled(&racl_ctrl, irq, true));
    }
    status_default_mask >>= 1;

    // This avoids a race where *irq_serviced is read before
    // entering the ISR.
    IBEX_SPIN_FOR(racl_ctrl_irq_serviced == irq, 1);
    LOG_INFO("IRQ %d from racl_ctrl is serviced.", irq);
  }
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 17 && 17 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralSocProxy;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 18 && 18 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralSpiDevice;
  status_default_mask = 0x0;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 19 && 19 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralSpiHost0;
  status_default_mask = 0x0;
  ...
  // non-DV setups.
  if (kDeviceType == kDeviceSimDV) {
    peripheral_expected = kTopDarjeelingNoIbexPlicPeripheralUart0;
...
bool test_main(void) {
  ...
  rv_plic_testutils_irq_range_enable(
      &plic, kHart, kTopDarjeeling_no_ibexPlicIrqIdNone + 1, kTopDarjeeling_no_ibexPlicIrqIdLast);
