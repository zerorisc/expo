...
static void init_peripherals(void) {
  ...
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_AES_BASE_ADDR);
  ...
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_CLKMGR_AON_BASE_ADDR);
  ...
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_LC_CTRL_REGS_BASE_ADDR);
  ...
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_PINMUX_AON_BASE_ADDR);
  ...
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_ROM_CTRL0_REGS_BASE_ADDR);
  CHECK_DIF_OK(dif_rom_ctrl_init(base_addr, &rom_ctrl0));

  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_ROM_CTRL1_REGS_BASE_ADDR);
  CHECK_DIF_OK(dif_rom_ctrl_init(base_addr, &rom_ctrl1));

  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_RSTMGR_AON_BASE_ADDR);
  ...
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_SOC_DBG_CTRL_CORE_BASE_ADDR);
  ...
  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_SRAM_CTRL_MAIN_REGS_BASE_ADDR);
  CHECK_DIF_OK(dif_sram_ctrl_init(base_addr, &sram_ctrl_main));

  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_SRAM_CTRL_MBOX_REGS_BASE_ADDR);
  CHECK_DIF_OK(dif_sram_ctrl_init(base_addr, &sram_ctrl_mbox));

  base_addr = mmio_region_from_addr(TOP_DARJEELING_NO_IBEX_SRAM_CTRL_RET_AON_REGS_BASE_ADDR);
...
static void trigger_alert_test(void) {
  ...
  for (dif_ac_range_check_alert_t i = 0; i < 2; ++i) {
    ...
    exp_alert = kTopDarjeelingNoIbexAlertIdAcRangeCheckRecovCtrlUpdateErr + i;
  ...
  for (dif_aes_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdAesRecovCtrlUpdateErr + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_aon_timer_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdAonTimerAonFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_clkmgr_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdClkmgrAonRecovFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_csrng_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdCsrngRecovAlert + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_dma_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdDmaFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_edn_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdEdn0RecovAlert + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_edn_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdEdn1RecovAlert + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_entropy_src_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdEntropySrcRecovAlert + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_gpio_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdGpioFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_hmac_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdHmacFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_i2c_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdI2c0FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_keymgr_dpe_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdKeymgrDpeRecovOperationErr + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_kmac_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdKmacRecovOperationErr + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_lc_ctrl_alert_t i = 0; i < 3; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdLcCtrlFatalProgError + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_mbx_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdMbx0FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_mbx_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdMbx1FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_mbx_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdMbx2FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_mbx_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdMbx3FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_mbx_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdMbx4FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_mbx_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdMbx5FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_mbx_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdMbx6FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_mbx_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdMbxJtagFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_mbx_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdMbxPcie0FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_mbx_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdMbxPcie1FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_otbn_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdOtbnFatal + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  if (kBootStage != kBootStageOwner) {
    ...
    for (dif_otp_ctrl_alert_t i = 0; i < 5; ++i) {
      ...
      // Verify that alert handler received it.
      exp_alert = kTopDarjeelingNoIbexAlertIdOtpCtrlFatalMacroError + i;
      CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
          &alert_handler, exp_alert, &is_cause));
      CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_pinmux_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdPinmuxAonFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_pwrmgr_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdPwrmgrAonFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_racl_ctrl_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdRaclCtrlFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_rom_ctrl_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdRomCtrl0Fatal + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_rom_ctrl_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdRomCtrl1Fatal + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_rstmgr_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdRstmgrAonFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_rv_plic_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdRvPlicFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_soc_dbg_ctrl_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdSocDbgCtrlFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_soc_proxy_alert_t i = 0; i < 29; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdSocProxyFatalAlertIntg + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_spi_device_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdSpiDeviceFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_spi_host_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdSpiHost0FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_sram_ctrl_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdSramCtrlMainFatalError + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_sram_ctrl_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdSramCtrlMboxFatalError + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_sram_ctrl_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdSramCtrlRetAonFatalError + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_uart_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopDarjeelingNoIbexAlertIdUart0FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
