...
`define DRIVE_CHIP_TL_HOST_IF(tl_name, inst_name, sig_name) \
     force ``tl_name``_tl_if.d2h = dut.top_daric2.u_``inst_name``.``sig_name``_i; \
     force dut.top_daric2.u_``inst_name``.``sig_name``_o = ``tl_name``_tl_if.h2d; \
     force dut.top_daric2.u_``inst_name``.clk_i = 0; \
...
`define DRIVE_CHIP_TL_DEVICE_IF(tl_name, inst_name, sig_name) \
     force ``tl_name``_tl_if.h2d = dut.top_daric2.u_``inst_name``.``sig_name``_i; \
     force dut.top_daric2.u_``inst_name``.``sig_name``_o = ``tl_name``_tl_if.d2h; \
     force dut.top_daric2.u_``inst_name``.clk_i = 0; \
     uvm_config_db#(virtual tl_if)::set(null, $sformatf("*env.%0s_agent", `"tl_name`"), "vif", \
                                        ``tl_name``_tl_if);
...
initial begin
  ...
  if (xbar_mode) begin
    ...
    $assertoff(0, tb);
    $asserton(0, tb.dut.top_daric2.u_xbar_main);
    $asserton(0, tb.dut.top_daric2.u_xbar_peri);
    $asserton(0, tb.dut.top_daric2.u_xbar_mbx);
    $asserton(0, tb.dut.top_daric2.u_xbar_dbg);


    // These are all zero-time: anything that consumes time go at the end.

    // bypass clkmgr, force clocks directly
    force tb.dut.top_daric2.u_xbar_main.clk_main_i = clk_main;
    force tb.dut.top_daric2.u_xbar_main.clk_fixed_i = clk_io_div4;
    force tb.dut.top_daric2.u_xbar_peri.clk_peri_i = clk_io_div4;
    force tb.dut.top_daric2.u_xbar_mbx.clk_mbx_i = clk_main;
    force tb.dut.top_daric2.u_xbar_dbg.clk_dbg_i = clk_main;
    force tb.dut.top_daric2.u_xbar_dbg.clk_peri_i = clk_io_div4;

    // bypass rstmgr, force resets directly
    force tb.dut.top_daric2.u_xbar_main.rst_main_ni = rst_n;
    force tb.dut.top_daric2.u_xbar_main.rst_fixed_ni = rst_n;
    force tb.dut.top_daric2.u_xbar_peri.rst_peri_ni = rst_n;
    force tb.dut.top_daric2.u_xbar_mbx.rst_mbx_ni = rst_n;
    force tb.dut.top_daric2.u_xbar_dbg.rst_dbg_ni = rst_n;
    force tb.dut.top_daric2.u_xbar_dbg.rst_peri_ni = rst_n;
