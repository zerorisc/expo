...
interface rom_ctrl_fsm_if ();
  ...
  task force_rom_select_bus_o(logic [3:0] value);
  ...
  function rom_ctrl_pkg::fsm_state_e get_fsm_state();
    return u_checker_fsm.state_q;
  endfunction

  // Wait until the FSM state is a value in the queue. Returns on the next negedge.
  task wait_for_fsm_state_inside(rom_ctrl_pkg::fsm_state_e states_to_visit[$]);
  ...
  task force_counter_addr(int unsigned addr);
