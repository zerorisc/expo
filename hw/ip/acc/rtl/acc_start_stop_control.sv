...
/**
 * State machine to handle actions that occur around the start and stop of ACC.
 *
...
#(
  ...
  `ASSERT_STATIC_LINT_ERROR(AccSecSkipUrndReseedAtStartNonDefault, SecSkipUrndReseedAtStart == 0)

  acc_start_stop_state_e state_q, state_d;
  ...
  if (SecSkipUrndReseedAtStart) begin: gen_skip_reseed
    ...
    assign skip_reseed_d = ((state_q == AccStartStopStateHalt) & start_i & ~stop);
  ...
  `PRIM_FLOP_SPARSE_FSM(u_state_regs, state_d, state_q,
      acc_start_stop_state_e, AccStartStopStateInitial)
  ...
  always_comb begin
    ...
    unique case (state_q)
      AccStartStopStateInitial: begin
        ...
        if (rma_request) begin
          ...
          state_d = AccStartStopSecureWipeWdrUrnd;
      ...
      AccStartStopStateHalt: begin
        ...
        if (stop && !rma_request) begin
          state_d = AccStartStopStateLocked;
          ...
          if (rma_request) begin
            ...
            state_d           = AccStartStopStateUrndRefresh;
          end
        end
      end
      AccStartStopStateUrndRefresh: begin
        ...
        if (stop) begin
          ...
          if (mubi4_test_false_strict(wipe_after_urnd_refresh_q)) begin
            ...
            if (urnd_reseed_ack_i || skip_reseed_q) begin
              state_d = AccStartStopStateRunning;
            end
          end else begin
      ...
      AccStartStopStateRunning: begin
      ...
      AccStartStopSecureWipeWdrUrnd: begin
        urnd_advance_o        = 1'b1;
        addr_cnt_inc          = 1'b1;
        sec_wipe_wdr_o        = 1'b1;
        ...
        if (addr_cnt_q == 6'b100000) begin
          ...
          sec_wipe_wdr_o      = 1'b0;
          sec_wipe_wdr_urnd_o = 1'b0;
          state_d = AccStartStopSecureWipeAccModBaseUrnd;
        end
      end
      ...
      AccStartStopSecureWipeAccModBaseUrnd: begin
        urnd_advance_o        = 1'b1;
        addr_cnt_inc          = 1'b1;
        allow_secure_wipe     = 1'b1;
        expect_secure_wipe    = 1'b1;
      ...
      AccStartStopSecureWipeComplete: begin
        urnd_advance_o = 1'b1;
        rma_ack_d = rma_req_i;
        state_d = should_lock_d ? AccStartStopStateLocked : AccStartStopStateHalt;
        wipe_after_urnd_refresh_d = MuBi4False;
      end
      AccStartStopStateLocked: begin
    ...
    if (urnd_reseed_ack_i &&
        !(state_q inside {AccStartStopStateInitial, AccStartStopStateUrndRefresh})) begin
  ...
  assign init_sec_wipe_done_d = (state_q == AccStartStopSecureWipeComplete) ? 1'b1 : // set
  ...
  assign done_o = ((state_q == AccStartStopSecureWipeComplete && init_sec_wipe_done_q) ||
                   (stop && (state_q == AccStartStopStateUrndRefresh) &&
                    mubi4_test_false_strict(wipe_after_urnd_refresh_q)) ||
                   (spurious_urnd_ack_error && !(state_q inside {AccStartStopStateHalt,
                                                                 AccStartStopStateLocked}) &&
  ...
  // loop through the escalation signal that our fatal_error_o causes acc_core to pass to the
  // controller.
  ...
  `ASSERT(StartStopStateValid_A,
      state_q inside {AccStartStopStateInitial,
                      AccStartStopStateHalt,
                      AccStartStopStateUrndRefresh,
                      AccStartStopStateRunning,
                      AccStartStopSecureWipeWdrUrnd,
                      AccStartStopSecureWipeAccModBaseUrnd,
                      AccStartStopSecureWipeAllZero,
                      AccStartStopSecureWipeComplete,
                      AccStartStopStateLocked})

  `ASSERT(StartSecureWipeImpliesRunning_A,
          $rose(secure_wipe_req_i) |-> (state_q == AccStartStopStateRunning))
