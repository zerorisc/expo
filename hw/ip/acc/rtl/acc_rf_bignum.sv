 ...
 *
 * This wraps two implementations, one for FPGA (acc_rf_base_fpga) implementation the other for
 * ASIC (acc_rf_base_ff).
 *
...
#(
  ...
  if (RegFile == RegFileFF) begin : gen_rf_bignum_ff
    acc_rf_bignum_ff u_acc_rf_bignum_inner (
      .clk_i,
      .rst_ni,
  ...
  // secure wipe.
  `ASSERT(AccRfBignumRdAKnown, rd_en_a_i && !rd_en_a_mismatch |-> !$isunknown(rd_data_a_intg_o))
  `ASSERT(AccRfBignumRdBKnown, rd_en_b_i && !rd_en_b_mismatch |-> !$isunknown(rd_data_b_intg_o))
endmodule
