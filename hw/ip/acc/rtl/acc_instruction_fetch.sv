...
/**
 * ACC Instruction Fetch Unit
 *
 * Fetch an instruction from the instruction memory.
 */
module acc_instruction_fetch
  import acc_pkg::*;
#(
  ...
  always_comb begin
    ...
    if (!insn_fetch_req_valid_raw_i) begin
      ...
      // scenario ACC is stalled and will eventually want the prefetched instruction.
      ...
      if (insn_is_branch(imem_rdata_i[31:0])) begin
        ...
        //
        // For x-prop reasons we do not suppress the imem_req_o here. When ACC executes an
        ...
        // imem_rdata_i is X, as ACC has halted following the error this doesn't cause a problem.
