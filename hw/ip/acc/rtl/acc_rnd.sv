...
/**
 * ACC random number coordination
 *
...
//                                                                                                //
// This is an initial prototype of the random number functionality in ACC. Details are still     //
// under discussion and subject to change. It has not yet been verified this provides the         //
// necessary guarantees required for the various uses of random numbers in ACC software.         //
////////////////////////////////////////////////////////////////////////////////////////////////////

module acc_rnd import acc_pkg::*;
#(
  ...
  // RND becomes valid when EDN request completes and provides new bits. Valid is cleared when ACC
  // starts a new run (opn_start_i) or when ACC reads RND (rnd_req_complete).
  assign rnd_valid_d =
  ...
  // harmless. However, a prefetch may still be outstanding from the last ACC run which may have
  // used a different configuration for EDN, CSRNG or the entropy source. At the start of a new
  // ACC run, RND data is thus always invalidated and outstanding prefetches are marked such that
