...
/**
 * Scramble control for ACC
 *
 * This provides a key and nonce for scrambling the ACC IMem and DMem. The OTP
 * key interface is used to request a new key and nonce when they are requested.
 */
module acc_scramble_ctrl
  import acc_pkg::*;
#(
  // Default seed and nonce for scrambling
  parameter otp_ctrl_pkg::acc_key_t   RndCnstAccKey   = acc_pkg::RndCnstAccKeyDefault,
  parameter otp_ctrl_pkg::acc_nonce_t RndCnstAccNonce = acc_pkg::RndCnstAccNonceDefault
) (
  // ACC clock
  input clk_i,
  input rst_ni,
  ...
  output otp_ctrl_pkg::acc_key_t acc_dmem_scramble_key_o,
  output acc_dmem_nonce_t        acc_dmem_scramble_nonce_o,
  output logic                    acc_dmem_scramble_valid_o,
  output logic                    acc_dmem_scramble_key_seed_valid_o,

  output otp_ctrl_pkg::acc_key_t acc_imem_scramble_key_o,
  output acc_imem_nonce_t        acc_imem_scramble_nonce_o,
  output logic                    acc_imem_scramble_valid_o,
  output logic                    acc_imem_scramble_key_seed_valid_o,

  input  logic                    acc_dmem_scramble_sec_wipe_i,
  input  otp_ctrl_pkg::acc_key_t acc_dmem_scramble_sec_wipe_key_i,
  input  logic                    acc_imem_scramble_sec_wipe_i,
  input  otp_ctrl_pkg::acc_key_t acc_imem_scramble_sec_wipe_key_i,

  output logic acc_dmem_scramble_key_req_busy_o,
  output logic acc_imem_scramble_key_req_busy_o,
  ...
  otp_ctrl_pkg::acc_key_t dmem_key_q, dmem_key_d;
  otp_ctrl_pkg::acc_key_t imem_key_q, imem_key_d;

  acc_dmem_nonce_t dmem_nonce_q;
  acc_imem_nonce_t imem_nonce_q;

  logic                      otp_key_req, otp_key_ack;
  otp_ctrl_pkg::acc_key_t   otp_key;
  otp_ctrl_pkg::acc_nonce_t otp_nonce;
  logic                      otp_key_seed_valid;

  assign dmem_key_d = dmem_key_sel_otp ? otp_key : acc_dmem_scramble_sec_wipe_key_i;
  assign imem_key_d = imem_key_sel_otp ? otp_key : acc_imem_scramble_sec_wipe_key_i;

  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      dmem_key_q <= RndCnstAccKey;
  ...
  always_comb begin
    ...
    if (acc_dmem_scramble_sec_wipe_i && state_q != ScrambleCtrlDmemReq) begin
    ...
    if (acc_imem_scramble_sec_wipe_i && state_q != ScrambleCtrlImemReq) begin
  ...
  assign acc_dmem_scramble_key_req_busy_o =
    (state_d == ScrambleCtrlDmemReq) | dmem_scramble_req_pending_d;

  assign acc_imem_scramble_key_req_busy_o =
    (state_d == ScrambleCtrlImemReq) | imem_scramble_req_pending_d;

  prim_sync_reqack_data #(
    .Width($bits(otp_ctrl_pkg::acc_otp_key_rsp_t)-1),
    .EnRstChks(1'b1),
    .DataSrc2Dst(1'b0)
  ) u_otp_key_req_sync (
    .clk_src_i (clk_i),
    ...
    .dst_req_o (acc_otp_key_o.req),
    .dst_ack_i (acc_otp_key_i.ack),
    .data_i    ({acc_otp_key_i.key,
                 acc_otp_key_i.nonce,
                 acc_otp_key_i.seed_valid}),
    .data_o    ({otp_key,
                 otp_nonce,
                 otp_key_seed_valid})
  );

  assign acc_dmem_scramble_key_o            = dmem_key_q;
  assign acc_dmem_scramble_nonce_o          = dmem_nonce_q;
  assign acc_dmem_scramble_valid_o          = dmem_key_valid_q;
  assign acc_dmem_scramble_key_seed_valid_o = dmem_key_seed_valid_q;

  assign acc_imem_scramble_key_o            = imem_key_q;
  assign acc_imem_scramble_nonce_o          = imem_nonce_q;
  assign acc_imem_scramble_valid_o          = imem_key_valid_q;
  assign acc_imem_scramble_key_seed_valid_o = imem_key_seed_valid_q;

  `ASSERT(AccScrambleCtrlLocalEscCntrMeasure_A, state_error_o |=> state_q == ScrambleCtrlError)
