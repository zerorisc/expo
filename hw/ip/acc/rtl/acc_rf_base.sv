 ...
 *
 * This wraps two implementations, one for FPGA (acc_rf_base_fpga)
 * implementation the other for ASIC (acc_rf_base_ff).
 *
...
#(
  ...
  // there's nothing on the call stack. ACC will raise an error when we read from an empty call
  ...
  // secure wipe.
  `ASSERT(AccRfBaseRdAKnown, rd_en_a_i && !pop_stack_a |-> !$isunknown(rd_data_a_raw_intg))
  `ASSERT(AccRfBaseRdBKnown, rd_en_b_i && !pop_stack_b |-> !$isunknown(rd_data_b_raw_intg))
