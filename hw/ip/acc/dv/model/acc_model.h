...
#ifndef OPENTITAN_HW_IP_ACC_DV_MODEL_ACC_MODEL_H_
#define OPENTITAN_HW_IP_ACC_DV_MODEL_ACC_MODEL_H_
...
class AccModel {
 public:
  ...
  AccModel(const std::string &mem_scope, const std::string &design_scope);
  ~AccModel();
  ...
  int take_loop_warps(const AccMemUtil &memutil);
  ...
  // checker. If the model has finished, writes acc.ERR_BITS to *err_bits.
  ...
  // acc_model framework as a convenient connection between SystemVerilog and
 ...
 private:
  ...
  AccMemUtil mem_util_;
  std::string design_scope_;

  bool stack_check_enabled_ = true;
};

#endif  // OPENTITAN_HW_IP_ACC_DV_MODEL_ACC_MODEL_H_
