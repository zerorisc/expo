...
#(
  ...
  // Scope of an RTL ACC implementation (for DPI). This should be give the scope for the top-level
  ...
  input  logic [7:0]         cmd_i,    // CMD register for ACC commands
...
`include "acc_model_dpi.svh"
  ...
  // Currently, the ACC core model doesn't ever set the lock_immediately_d/q signals. However,
  // the ACC model agent provides a function (see lock_immediately() in acc_model_if.sv) to
  ...
  // RND Request starts if ACC Model raises rnd_req_start while we are not
  ...
  assign is_idle = acc_pkg::status_e'(status_o) == StatusIdle;

  // URND Reseeding is done twice as part of every secure wipe: once before the secure wipe and once
  // after a first wipe with random data.  A secure wipe happens after reset and when ACC receives
  // the `EXECUTE` command.
  typedef enum logic [2:0] {
    AccCoreModelUrndStateReset,
    AccCoreModelUrndStateAwaitInitialAck,
    AccCoreModelUrndStateAwaitWipe,
    AccCoreModelUrndStateAwaitSecondAck,
    AccCoreModelUrndStateAwaitStart,
    AccCoreModelUrndStateAwaitPostStartAck,
    AccCoreModelUrndStateAwaitPostExecSecWipe
  } urnd_state_e;
  ...
  always_comb begin
    ...
    unique case (urnd_state_q)
      AccCoreModelUrndStateReset: begin
        urnd_state_d = AccCoreModelUrndStateAwaitInitialAck;
      end

      AccCoreModelUrndStateAwaitInitialAck: begin
        edn_urnd_o = 1'b1;
        if (edn_urnd_cdc_done_i) begin
          wipe_cyc_cnt_d = wipe_cyc_cnt_t'(WIPE_CYCLES);
          urnd_state_d   = AccCoreModelUrndStateAwaitWipe;
        end
      end

      AccCoreModelUrndStateAwaitWipe: begin
        wipe_cyc_cnt_d = wipe_cyc_cnt_q - 1;
        if (wipe_cyc_cnt_q == '0) begin
          edn_urnd_o   = 1'b1;
          urnd_state_d = AccCoreModelUrndStateAwaitSecondAck;
        end
      end

      AccCoreModelUrndStateAwaitSecondAck: begin
        edn_urnd_o = 1'b1;
        if (edn_urnd_cdc_done_i) begin
          urnd_state_d = AccCoreModelUrndStateAwaitStart;
        end
      end

      AccCoreModelUrndStateAwaitStart: begin
        if (start_q) begin
          urnd_state_d = AccCoreModelUrndStateAwaitPostStartAck;
        end
      end

      AccCoreModelUrndStateAwaitPostStartAck: begin
        edn_urnd_o = 1'b1;
        if (edn_urnd_cdc_done_i) begin
          urnd_state_d = AccCoreModelUrndStateAwaitPostExecSecWipe;
        end
      end

      AccCoreModelUrndStateAwaitPostExecSecWipe: begin
      ...
      default: urnd_state_d = AccCoreModelUrndStateReset;
    endcase

    if (lock_immediately_q) begin
      urnd_state_d = AccCoreModelUrndStateReset;
    end
  end
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      start_q            <= 1'b0;
      urnd_state_q       <= AccCoreModelUrndStateReset;
      wipe_cyc_cnt_q     <= '0;
    end else begin
      start_q        <= start_d;
  ...
  always_ff @(posedge clk_edn_i or negedge rst_edn_ni) begin
    if (!rst_edn_ni) begin
      failed_rnd_step <= 0;
      failed_urnd_step <= 0;
      failed_edn_flush <= (acc_model_edn_flush(model_handle) != 0);
    end else begin
      if (edn_rnd_i.edn_ack) begin
        failed_rnd_step <= (acc_model_edn_rnd_step(model_handle, edn_rnd_i.edn_bus,
                                                    ~edn_rnd_i.edn_fips) != 0);
      end
      if (edn_urnd_i.edn_ack) begin
        failed_urnd_step <= (acc_model_edn_urnd_step(model_handle, edn_urnd_i.edn_bus) != 0);
      end
    end
  end
  ...
  always @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      failed_reset <= (acc_model_reset(model_handle,
                                        status_q,
                                        insn_cnt_q,
                                        rnd_req_start_q,
                                        raw_err_bits_q,
      ...
      if (!initial_secure_wipe_started) begin
        failed_initial_secure_wipe <= (acc_model_initial_secure_wipe(model_handle) != 0);
        initial_secure_wipe_started <= 1;
      end
      if (new_escalation) begin
        // Setting LIFECYCLE_ESCALATION bit
        failed_lc_escalate <= (acc_model_send_err_escalation(model_handle,
                                                              32'd1 << 22,
                                                              1'b0)
                               != 0);
      end
      if (rma_req_changed) begin
        failed_lc_rma_req <= (acc_model_set_rma_req(model_handle, rma_req_fifo[1]) != 0);
      end
      if (!$stable(keymgr_key_i) || $rose(rst_ni)) begin
        failed_keymgr_value <= (acc_model_set_keymgr_value(model_handle,
                                                            keymgr_key_i.key[0],
                                                            keymgr_key_i.key[1],
                                                            keymgr_key_i.valid) != 0);
      end
      if (edn_urnd_cdc_done_i) begin
        failed_urnd_cdc <= (acc_model_urnd_cdc_done(model_handle) != 0);
      end
      if (edn_rnd_cdc_done_i) begin
        failed_rnd_cdc <= (acc_model_rnd_cdc_done(model_handle) != 0);
      end
      if (otp_key_cdc_done_i) begin
        failed_otp_key_cdc <= (acc_model_otp_key_cdc_done(model_handle) != 0);
      end
      if (step_iss) begin
        model_state <= acc_model_step(model_handle,
                                       model_state,
                                       cmd,
                                       status_q,
                                       insn_cnt_q,
  ...
  always_ff @(negedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      if (check_due) begin
        failed_check <= (acc_model_check(model_handle, check_mismatch_d) == 0);
  ...
  // multiple ACC instances, so it would be nice to get it right.
  bind acc_rf_base_ff acc_rf_snooper_if #(
    .Width           (BaseIntgWidth),
    .Depth           (NGpr),
    .IntegrityEnabled(1)
  ) u_snooper (
  ...
  bind acc_rf_bignum_ff acc_rf_snooper_if #(
    .Width           (ExtWLEN),
    .Depth           (NWdr),
    .IntegrityEnabled(1)
  ) u_snooper (
  ...
  bind acc_rf_base acc_stack_snooper_if #(.StackIntgWidth(39), .StackWidth(32), .StackDepth(8))
  ...
  // Derive a "done" signal. This should trigger for a single cycle when ACC finishes its work.
  // It's analogous to the done_o signal on acc_core, but this signal is delayed by a single cycle
  // (hence its name is done_r_o).
  bit acc_model_busy, acc_model_busy_r;
  assign acc_model_busy = !(status_q inside {StatusIdle, StatusLocked}) & init_sec_wipe_done_i;
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      acc_model_busy_r <= 1'b0;
    end else begin
      acc_model_busy_r <= acc_model_busy;
    end
  end
  assign done_rr_o = acc_model_busy_r & ~acc_model_busy;

  // Make stop_pc available over DPI. This is handy for Verilator simulations (where the top-level
  // is in C++).
  export "DPI-C" function acc_core_get_stop_pc;
  function automatic int acc_core_get_stop_pc();
    return stop_pc_q;
  endfunction
