...
#ifndef OPENTITAN_HW_IP_ACC_DV_MODEL_ACC_TRACE_ENTRY_H_
#define OPENTITAN_HW_IP_ACC_DV_MODEL_ACC_TRACE_ENTRY_H_
...
// This models a body line in an ACC trace entry (type '<', '>', 'R' or 'W').
...
class AccTraceBodyLine {
 public:
  ...
  bool operator==(const AccTraceBodyLine &other) const;
...
class AccTraceEntry {
 public:
  ...
  bool compare_rtl_iss_entries(const AccTraceEntry &other,
                               bool no_sec_wipe_data_chk,
                               std::string *err_desc) const;

  void print(const std::string &indent, std::ostream &os) const;

  void take_writes(const AccTraceEntry &other, bool other_first);
  ...
  bool is_compatible(const AccTraceEntry &other) const;
 ...
 protected:
  ...
  std::map<std::string, std::vector<AccTraceBodyLine>> writes_;
};

class AccIssTraceEntry : public AccTraceEntry {
 public:
...
#endif  // OPENTITAN_HW_IP_ACC_DV_MODEL_ACC_TRACE_ENTRY_H_
