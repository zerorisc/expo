...
static std::unique_ptr<AccTraceChecker> trace_checker;

AccTraceChecker::AccTraceChecker()
    : rtl_started_(false),
      rtl_pending_(false),
      iss_started_(false),
      iss_pending_(false),
  ...
  AccTraceSource::get().AddListener(this);
}

AccTraceChecker::~AccTraceChecker() {
  if (!done_) {
    std::cerr
        << ("WARNING: Destroying AccTraceChecker object with an "
            "unfinished operation.\n");
  }
}

AccTraceChecker &AccTraceChecker::get() {
  if (!trace_checker) {
    trace_checker.reset(new AccTraceChecker());
  }
...
void AccTraceChecker::AcceptTraceString(const std::string &trace,
  ...
  done_ = false;
  AccTraceEntry trace_entry;
  if (!trace_entry.from_rtl_trace(trace)) {
    seen_err_ = true;
    return;
  }
  if (trace_entry.trace_type() == AccTraceEntry::Invalid) {
  ...
  if (trace_entry.trace_type() == AccTraceEntry::Stray) {
...
bool AccTraceChecker::OnIssTrace(const std::vector<std::string> &lines) {
  assert(!(rtl_pending_ && iss_pending_));
...
const AccIssTraceEntry::IssData *AccTraceChecker::PopIssData() {
  if (!last_data_vld_)
    return nullptr;
...
void AccTraceChecker::set_no_sec_wipe_chk() { no_sec_wipe_data_chk_ = true; }

bool AccTraceChecker::MatchPair() {
...
//    acc_trace_checker_pop_iss_insn(output bit [31:0] insn_addr,
...
extern "C" unsigned char acc_trace_checker_pop_iss_insn(
  ...
  const AccIssTraceEntry::IssData *iss_data =
      AccTraceChecker::get().PopIssData();
  if (!iss_data)
    return 0;
