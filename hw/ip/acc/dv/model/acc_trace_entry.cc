...
bool AccTraceBodyLine::operator==(const AccTraceBodyLine &other) const {
...
bool AccTraceEntry::from_rtl_trace(const std::string &trace) {
...
void AccTraceEntry::print(const std::string &indent, std::ostream &os) const {
  os << indent << hdr_ << "\n";
...
void AccTraceEntry::take_writes(const AccTraceEntry &other,
  ...
  for (const auto &pr : other.writes_) {
    ...
    if (other_first) {
      ...
      std::vector<AccTraceBodyLine> tmp(pr.second);
...
bool AccTraceEntry::is_compatible(const AccTraceEntry &prev) const {
...
bool AccTraceEntry::is_partial() const {
  return ((trace_type_ == AccTraceEntry::Stall) ||
          (trace_type_ == AccTraceEntry::WipeInProgress));
}

bool AccTraceEntry::is_final() const {
  return ((trace_type_ == AccTraceEntry::Exec) ||
          (trace_type_ == AccTraceEntry::WipeComplete) ||
          (trace_type_ == AccTraceEntry::Stray));
}
...
AccTraceEntry::trace_type_t AccTraceEntry::hdr_to_trace_type(
...
bool AccIssTraceEntry::from_iss_trace(const std::vector<std::string> &lines) {
