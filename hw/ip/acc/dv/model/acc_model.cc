...
extern "C" {
int acc_rf_peek(int index, svBitVecVal *val);
int acc_stack_element_peek(int index, svBitVecVal *val);
}
...
// Values come from acc_pkg to signify the value for specific operations.
#define CMD_EXECUTE 0xD8
#define CMD_SECWIPE_DMEM 0xC3
#define CMD_SECWIPE_IMEM 0x1E

// Values of the `STATUS` register, as defined in `acc.hjson`.
...
static std::array<T, 32> get_rtl_regs(const std::string &reg_scope) {
  ...
  // acc_rf_peek passes data as a packed array of svBitVecVal words (for a
...
static std::vector<T> get_stack(const std::string &stack_scope) {
  ...
  // acc_stack_element_peek passes data as a packed array of svBitVecVal words
  ...
  while (1) {
    int peek_result = acc_stack_element_peek(i, buf);
...
int AccModel::take_loop_warps(const AccMemUtil &memutil) {
...
int AccModel::start_operation(command_t command) {
  ISSWrapper *iss = ensure_wrapper();
  if (!iss)
    return -1;
...
int AccModel::edn_urnd_step(svLogicVecVal *edn_urnd_data /* logic [31:0] */) {
  ISSWrapper *iss = ensure_wrapper();
  if (!iss)
    return -1;
...
int AccModel::edn_urnd_cdc_done() {
  ISSWrapper *iss = ensure_wrapper();
  if (!iss)
    return -1;
...
int AccModel::check() const {
  ...
  good &= AccTraceChecker::get().Finish();
...
int AccModel::set_software_errs_fatal(unsigned char new_val) {
  ISSWrapper *iss = ensure_wrapper();
  if (!iss)
    return -1;
...
int AccModel::set_no_sec_wipe_chk() {
  AccTraceChecker::get().set_no_sec_wipe_chk();
  return 0;
}
...
int AccModel::set_rma_req(svBitVecVal *rma_req /* bit [3:0] */) {
  ISSWrapper *iss = ensure_wrapper();
  if (!iss)
    return -1;
...
bool AccModel::is_at_start_of_wipe() const {
  ISSWrapper *iss = iss_.get();
  return iss && iss->get_mirrored().wipe_start;
}

ISSWrapper *AccModel::ensure_wrapper() {
  if (!iss_) {
    try {
...
Ecc32MemArea::EccWords AccModel::get_sim_memory(bool is_imem) const {
...
bool AccModel::check_dmem(ISSWrapper &iss) const {
  const MemArea &dmem = mem_util_.GetMemArea(false);
...
bool AccModel::check_regs(ISSWrapper &iss) const {
  std::string base_scope =
      design_scope_ +
      ".u_acc_rf_base.gen_rf_base_ff.u_acc_rf_base_inner.u_snooper";
  std::string wide_scope =
      design_scope_ +
      ".u_acc_rf_bignum.gen_rf_bignum_ff.u_acc_rf_bignum_inner.u_snooper";
...
bool AccModel::check_call_stack(ISSWrapper &iss) const {
  std::string call_stack_snooper_scope =
      design_scope_ + ".u_acc_rf_base.u_call_stack_snooper";
...
int AccModel::initial_secure_wipe() {
  ISSWrapper *iss = ensure_wrapper();
  if (!iss)
    return -1;
...
AccModel *acc_model_init(const char *mem_scope, const char *design_scope) {
  ...
  return new AccModel(mem_scope, design_scope);
}

void acc_model_destroy(AccModel *model) { delete model; }
...
unsigned acc_model_step(AccModel *model, unsigned model_state,
  ...
  if (*status == STATUS_IDLE) {
    // ACC only executes commands if STATUS is IDLE.
    switch (*cmd) {
      case CMD_EXECUTE:
        result = model->start_operation(AccModel::Execute);
        new_state_bits = RUNNING_BIT;
        break;
      case CMD_SECWIPE_DMEM:
        result = model->start_operation(AccModel::DmemWipe);
        new_state_bits = RUNNING_BIT;
        break;
      case CMD_SECWIPE_IMEM:
        result = model->start_operation(AccModel::ImemWipe);
        new_state_bits = RUNNING_BIT;
        break;
    }
  }
