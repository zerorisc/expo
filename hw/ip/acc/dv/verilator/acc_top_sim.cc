...
extern "C" {
extern unsigned int acc_base_call_stack_get_size();
extern unsigned int acc_base_call_stack_get_element(int index);
extern unsigned int acc_base_reg_get(int index);
extern unsigned int acc_bignum_reg_get(int index, int quarter);
extern svBit acc_err_get();
extern int acc_core_get_stop_pc();
}

/**
 * SimCtrlExtension that adds a '--acc-trace-file' command line option. If set
 * it sets up a LogTraceListener that will dump out the trace to the given log
 * file
 */
class AccTraceUtil : public SimCtrlExtension {
 private:
  ...
  bool SetupTraceLog(const std::string &log_filename) {
    try {
      ...
      AccTraceSource::get().AddListener(log_trace_listener_.get());
      return true;
 ...
 public:
  virtual bool ParseCLIArguments(int argc, char **argv, bool &exit_app) {
    const struct option long_options[] = {
        {"acc-trace-file", required_argument, nullptr, 'l'},
  ...
  ~AccTraceUtil() {
    if (log_trace_listener_)
      AccTraceSource::get().RemoveListener(log_trace_listener_.get());
  }
};

static acc_top_sim *verilator_top;
static AccMemUtil acc_memutil("TOP.acc_top_sim");

int main(int argc, char **argv) {
  VerilatorMemUtil memutil(&acc_memutil);
  AccTraceUtil traceutil;

  acc_top_sim top;
  // Make the acc_top_sim object visible to AccTopApplyLoopWarp.
  ...
  std::cout << "Simulation of ACC" << std::endl
            << "==================" << std::endl
            << std::endl;
  ...
  svSetScope(svGetScopeFromName("TOP.acc_top_sim"));

  svBit model_err = acc_err_get();
  if (model_err) {
    return 1;
  }

  int exp_stop_pc = acc_memutil.GetExpEndAddr();
  if (exp_stop_pc >= 0) {
    SVScoped core_scope("TOP.acc_top_sim.u_acc_core_model");
    int act_stop_pc = acc_core_get_stop_pc();
...
// the ELF file.
extern "C" int AccTopInstallLoopWarps() {
  // Cast to the right base class of acc_top_sim. Otherwise, you can't access
  // the "acc_top_sim" member because you get the derived class's constructor
  // by accident.
  Vacc_top_sim &top = *verilator_top;

  // Grab the model handle from the acc_core_model module. This should have
  // been initialised by now because it gets set up in an initial block and
  // this code doesn't run until the first clock edge.
  auto sv_model_handle = top.acc_top_sim->u_acc_core_model->model_handle;

  // sv_model_handle will be some integer type. Check it's nonzero and, if so,
  // convert it to an AccModel*.
  assert(sv_model_handle != 0);

  AccModel *model_handle = (AccModel *)sv_model_handle;

  if (model_handle->take_loop_warps(acc_memutil) != 0) {
...
// in the ELF file.
extern "C" void AccTopApplyLoopWarp() {
  static std::vector<uint32_t> loop_count_stack;

  // See not in AccTopInstallLoopWarps for why this upcast is needed.
  Vacc_top_sim &top = *verilator_top;

  auto loop_controller =
      top.acc_top_sim->u_acc_core->u_acc_controller->u_acc_loop_controller;
  ...
  if (!loop_count_stack.empty()) {
    ...
    uint32_t new_cnt = acc_memutil.GetLoopWarp(insn_addr, old_cnt);
    if (old_cnt != new_cnt) {
...
extern "C" void AccTopDumpState() {
  std::cout << "Call Stack:" << std::endl;
  std::cout << "-----------" << std::endl;
  for (int i = 0; i < acc_base_call_stack_get_size(); ++i) {
    std::cout << std::setfill(' ') << "0x" << std::hex << std::setw(8)
              << std::setfill('0') << std::right
              << acc_base_call_stack_get_element(i) << std::endl;
  }
  ...
  for (int i = 2; i < 32; ++i) {
    std::cout << "x" << std::left << std::dec << std::setw(2)
              ...
              << std::setfill('0') << std::right << acc_base_reg_get(i)
              << std::endl;
  }
  ...
  for (int i = 0; i < 32; ++i) {
    ...
    std::cout << std::setw(8) << std::setfill('0') << std::right
              << acc_bignum_reg_get(i, 7) << "_";

    std::cout << std::setw(8) << std::setfill('0') << acc_bignum_reg_get(i, 6)
              << "_";

    std::cout << std::setw(8) << std::setfill('0') << acc_bignum_reg_get(i, 5)
              << "_";

    std::cout << std::setw(8) << std::setfill('0') << acc_bignum_reg_get(i, 4)
              << "_";

    std::cout << std::setw(8) << std::setfill('0') << acc_bignum_reg_get(i, 3)
              << "_";

    std::cout << std::setw(8) << std::setfill('0') << acc_bignum_reg_get(i, 2)
              << "_";

    std::cout << std::setw(8) << std::setfill('0') << acc_bignum_reg_get(i, 1)
              << "_";

    std::cout << std::setw(8) << std::setfill('0') << acc_bignum_reg_get(i, 0)
              << std::endl;
  }
}
