...
class AccMemUtil : public DpiMemUtil {
 public:
  ...
  AccMemUtil(const std::string &top_scope);
...
extern "C" {
AccMemUtil *AccMemUtilMake(const char *top_scope);
void AccMemUtilFree(AccMemUtil *mem_util);
...
svBit AccMemUtilLoadElf(AccMemUtil *mem_util, const char *elf_path);

// Loads an ELF file into the AccMemUtil object, but doesn't touch the
// simulated memory. Returns 1'b1 on success. Prints a message to stderr and
// returns 1'b0 on failure.
svBit AccMemUtilStageElf(AccMemUtil *mem_util, const char *elf_path);

// Returns the number of segments currently staged in imem/dmem.
int AccMemUtilGetSegCount(AccMemUtil *mem_util, svBit is_imem);
...
// did stop there.
//
// Note: This functionality doesn't provide any extra check of ACC itself.
