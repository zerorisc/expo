...
(
  ...
  input acc_reg2hw_t reg2hw,
  input acc_hw2reg_t hw2reg,
  input logic         done_i,

  input prim_mubi_pkg::mubi4_t idle_o_i,

  input logic acc_dmem_scramble_key_req_busy_i,
  input logic acc_imem_scramble_key_req_busy_i,
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      rotating_keys <= acc_dmem_scramble_key_req_busy_i | acc_imem_scramble_key_req_busy_i;
      done          <= done_i;
    end
  end
  ...
  assign cmd_operation = reg2hw.cmd.q inside {acc_pkg::CmdSecWipeImem,
                                              acc_pkg::CmdSecWipeDmem,
                                              acc_pkg::CmdExecute};

  assign start_req = reg2hw.cmd.qe && cmd_operation;

  assign do_start = start_req && (hw2reg.status.d == acc_pkg::StatusIdle);

  // Track whether ACC has completed its initial secure wipe.
  ...
  // Our model of whether ACC is running or not. We start on `do_start` once the initial secure
  ...
  // see cmd_start when we are running, need not be true: the host can do that if it likes and ACC
  ...
  //
  //  - If STATUS has value LOCKED and no secure wipe is running, we should not think ACC is
  ...
  assign running_or_locked = running_qq ||
                             busy_secure_wipe ||
                             status_q_i == acc_pkg::StatusLocked;
  ...
  `ASSERT(NotIdleIfLockedAndRotatingKeys_A,
          ((status_q_i == acc_pkg::StatusLocked) && keys_busy) |->
  ...
  `ASSERT(IdleIfLockedAndNotRotatingKeys_A,
          ...
          // to become true.
          ((status_q_i == acc_pkg::StatusLocked) && !keys_busy && !busy_secure_wipe) |-> ##[1:2]
          (idle_o_i == prim_mubi_pkg::MuBi4True))

  `ASSERT(NoStartKeyRotationWhenLocked_A,
          (status_q_i == acc_pkg::StatusLocked) |=> !$rose(keys_busy))

  `ASSERT(OnlyKeyRotationWhenRunningOrLocked_A,
          keys_busy |-> (running_q || (status_q_i == acc_pkg::StatusLocked)))

  `ASSERT(NotRunningWhenLocked_A,
          (status_q_i == acc_pkg::StatusLocked && !busy_secure_wipe) |->
          !running_d)

  // When ACC locks bus read data integrity is forced to the correct value for 0 data (so reads to
  // a locked ACC don't cause an integrity error). There is a small window where running_q is set
  // with status_q reporting 'StatusLocked'. So expected bus read data depends upon locked status
  // when running.
  `ASSERT(NoMemRdataWhenBusy_A,
    running_q && !(status_q_i == acc_pkg::StatusBusySecWipeInt) |->
      ((status_q_i == acc_pkg::StatusLocked) ?
