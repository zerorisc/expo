...
class AccTestHelper {
 public:
  ...
  AccTestHelper(const std::string dir_path) : dir_path_(dir_path) {}
...
// Make an ACC test helper
extern "C" AccTestHelper *AccTestHelperMake(const char *path) {
  assert(path != nullptr);
  return new AccTestHelper(path);
}

// Free an ACC test helper
extern "C" void AccTestHelperFree(AccTestHelper *helper) {
...
// reason), return zero.
extern "C" int AccTestHelperCountFilesInDir(AccTestHelper *helper) {
  assert(helper != nullptr);
