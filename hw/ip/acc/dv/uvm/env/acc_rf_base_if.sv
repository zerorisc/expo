...
//
// Bound into the acc_rf_base and used to help collect call stack information for coverage.

interface acc_rf_base_if (
  input       clk_i,
  input       rst_ni,

  // Signal names from the acc_rf_base module (where we are bound)
  ...
  function automatic acc_env_pkg::call_stack_flags_t get_call_stack_flags();
    return '{
              pop_a: pop_stack_a,
              pop_b: pop_stack_b,
  ...
  function automatic acc_env_pkg::stack_fullness_e get_call_stack_fullness();
    if (stack_full) begin
  ...
  function static void force_rf_base_rd_data_a_intg(
      input logic [acc_pkg::BaseIntgWidth-1:0] should_val
    );
    force u_acc_rf_base.rd_data_a_intg_o = should_val;
  endfunction
