...
class acc_scoreboard extends cip_base_scoreboard #(
    .CFG_T(acc_env_cfg),
    .RAL_T(acc_reg_block),
    .COV_T(acc_env_cov)
  );
  ...
  uvm_tlm_analysis_fifo #(acc_model_item) model_fifo;
  uvm_tlm_analysis_fifo #(acc_trace_item) trace_fifo;
  ...
  acc_model_item iss_trace_queue[$];
  acc_trace_item rtl_trace_queue[$];
  ...
  //       should be.
  //
  // To handle these three possibilities, we store a 34-bit acc_exp_read_data_t value with fields
  ...
  acc_exp_read_data_t exp_read_values [tl_source_t];

  // A flag that tracks the fact that we've seen a TL write to the CMD register that we expect to
  // start ACC. We track this because we derive the "start" signal in the model from an internal
  ...
  // The "locked" field is used to track whether ACC is "locked". For most operational state
  // tracking, we go through the ISS, but ACC can become locked without actually starting an
  ...
  task process_tl_addr(tl_seq_item item);
    ...
    acc_exp_read_data_t exp_read_data = '{upd: 1'b0, chk: 'x, val: 'x};
    ...
    if (item.is_write()) begin
      ...
      if (cfg.en_cov) begin
        cov.on_ext_csr_access(csr, acc_env_pkg::AccessSoftwareWrite, item.a_data, state);
      end
      ...
      case (csr.get_name())
        ...
        "cmd": begin
          ...
          // this tracking works.
          bit cmd_operation = item.a_data[7:0] inside {acc_pkg::CmdSecWipeImem,
                                                       acc_pkg::CmdSecWipeDmem,
                                                       acc_pkg::CmdExecute};
          if (cmd_operation && (model_status == acc_pkg::StatusIdle)) begin
        ...
        "ctrl": begin
          ...
          // write in testbench level and sending a signal to acc_core_model, which is not ideal
          // as well.
          if (item.is_write) begin
    ...
    // Otherwise, this is a read transaction. Fill in an acc_exp_read_data_t struct appropriately.
    case (csr.get_name())
      "intr_state": begin
  ...
  task process_tl_data(tl_seq_item item);
    ...
    acc_exp_read_data_t exp_read_data;
    ...
    if (cfg.en_cov) begin
      cov.on_ext_csr_access(csr, acc_env_pkg::AccessSoftwareRead, item.d_data,
  ...
  task process_model_fifo();
    ...
    forever begin
      ...
      case (item.item_type)
        AccModelStatus: begin
          bit was_executing = model_status inside {acc_pkg::StatusBusyExecute,
                                                   acc_pkg::StatusBusySecWipeInt};
          bit is_busy = acc_pkg::is_busy_status(status_e'(item.status));
          ...
          // register.
          if (model_status == acc_pkg::StatusIdle && is_busy) begin
          ...
          if ((item.status == acc_pkg::StatusLocked) &&
          ...
          if (was_executing && item.status == acc_pkg::StatusIdle && item.err_bits != 0) begin
            expect_alert("recov");
          end
        ...
        AccModelInsn: begin
          // The model agent's monitor should be configured to only emit AccModelInsn items if
          // coverage is enabled.
  ...
  function void pop_trace_queues();
    while ((iss_trace_queue.size() > 0) && (rtl_trace_queue.size() > 0)) begin
      acc_model_item iss_item = iss_trace_queue.pop_front();
      acc_trace_item rtl_item = rtl_trace_queue.pop_front();
  ...
  function void on_alert(string alert_name, alert_esc_agent_pkg::alert_esc_seq_item item);
    ...
    // Wait up to 4000 cycles for the prediction to come through (giving up on reset).  This is a
  ...
  virtual function void mem_compare(string ral_name, uvm_reg_addr_t addr, tl_seq_item item);
    // We can only compare the contents inside memories when the ACC is not busy executing
    // or wiping the memories
    if (model_status inside {acc_pkg::StatusIdle, acc_pkg::StatusBusySecWipeInt}) begin
      super.mem_compare(ral_name, addr, item);
    // Otherwise the contents will read out as zeros so compare expected memory with zero.
    end else begin
      `DV_CHECK_EQ(item.d_data, '0, "Memory read out nonzero value while ACC is not IDLE")
    end
  endfunction
  ...
  virtual task process_mem_read(tl_seq_item item, string ral_name);
    ...
    if (model_status == 'b1 && item.d_data != 0) begin
      `uvm_error(`gfn, "read data is non zero when memory is accessed while acc is busy")
    end
  endtask
