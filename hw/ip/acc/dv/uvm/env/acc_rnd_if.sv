...
//
// Bound into acc_rnd_module and used to help collect events for coverage

`include "prim_assert.sv"

interface acc_rnd_if (
  input logic clk_i,
  input logic rst_ni,

  // The signals below are all bound in with .* so the names match those in acc_rnd.sv
  ...
  //
  // The easiest way to spot these events is to look at the ports of the acc_rnd module. [RD RND]
  // happens when we see the rnd_req_i signal of the acc_rnd module go high. [WR RND_PREFETCH]
