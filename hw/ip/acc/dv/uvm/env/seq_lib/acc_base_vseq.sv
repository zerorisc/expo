...
//
// Base class for all ACC test sequences
//

class acc_base_vseq extends cip_base_vseq #(
  .CFG_T              (acc_env_cfg),
  .RAL_T              (acc_reg_block),
  .COV_T              (acc_env_cov),
  .VIRTUAL_SEQUENCER_T(acc_virtual_sequencer)
);
  `uvm_object_utils(acc_base_vseq)
  `uvm_object_new

  // "Running" flag to detect concurrent executions of run_acc()
  protected bit running_ = 1'b0;

  // The longest run seen in calls to start_running_acc. This gets reset to zero by load_elf (since
  ...
  // This counter is set in run_acc() if our operation has finished and we're trying to stop. It's
  ...
  protected task load_elf(string path, bit backdoor);
    ...
    `DV_CHECK_STD_RANDOMIZE_WITH_FATAL(wipe_cmd, wipe_cmd != acc_pkg::CmdExecute;)
    if (bogus_write) begin
      // Write a bogus value while we are in IDLE state for coverage.
      _send_bogus_cmd();
    end
    case (num_wipes)
      1: _run_acc_cmd(wipe_cmd);  // Run a random wipe command
      2: begin
        // First run a random wipe command
        _run_acc_cmd(wipe_cmd);
        ...
        if (wipe_cmd.name() == "CmdSecWipeDmem") _run_acc_cmd(acc_pkg::CmdSecWipeImem);
        else _run_acc_cmd(acc_pkg::CmdSecWipeDmem);
      end
      default: ;  // Do nothing
    endcase
    if (backdoor) begin
    ...
    // Pass loop warp rules that we've just loaded into acc_memutil into the model.
  ...
  protected function void load_elf_backdoor(string path);
    if (!AccMemUtilLoadElf(cfg.mem_util, path)) begin
  ...
  protected task load_elf_over_bus(string path);
    acc_loaded_word        to_load         [$];
    bit              [33:0] opns            [$];
    bit                     run_fast;
    semaphore               tx_sem = new(0);

    // First, tell AccMemUtil to stage the ELF. This reads the file and stashes away the segments
    // we need. If something goes wrong, it will print a message to stderr, so we can just fail.
    if (!AccMemUtilStageElf(cfg.mem_util, path)) begin
  ...
  protected task send_mem_operation(acc_loaded_word to_load[$], bit [1:0] op, bit [31:0] value);
    case (op)
      2'b00:
  ...
  protected function automatic void get_queue_entries(bit for_imem,
                                                      ref acc_loaded_word entries[$]);
    // Get the bus-accessible size of this memory (to make sure the number of loaded words makes
    // sense)
    int unsigned mem_size = for_imem ? ACC_IMEM_SIZE : ACC_DMEM_SIZE;

    // Iterate over the segments for this memory
    int seg_count = AccMemUtilGetSegCount(cfg.mem_util, for_imem);
    for (int seg_idx = 0; seg_idx < seg_count; seg_idx++) begin
      ...
      if (!AccMemUtilGetSegInfo(cfg.mem_util, for_imem, seg_idx, seg_off, seg_size)) begin
      ...
      for (bit [31:0] i = 0; i < seg_size; i++) begin
        ...
        acc_loaded_word entry;

        word_off = seg_off + i;

        if (!AccMemUtilGetSegData(cfg.mem_util, for_imem, word_off, data)) begin
        ...
        // AccMemUtil should have checked that this address was valid for the given memory, but it
  ...
  //
  // This is only used when ACC is either known to be locked or running (so it will ignore the
  ...
  protected task _send_random_cmd();
    ...
    `DV_CHECK_STD_RANDOMIZE_WITH_FATAL(val,
                                       val[7:0] dist {
                                         acc_pkg::CmdExecute     :/ 2,
                                         acc_pkg::CmdSecWipeDmem :/ 2,
                                         acc_pkg::CmdSecWipeImem :/ 2,
                                         [0 : $]                  :/ 1
                                       };)
  ...
  protected task _send_bogus_cmd();
    logic [31:0] val;
    `DV_CHECK_STD_RANDOMIZE_WITH_FATAL(val,
                                       !(val[7:0] inside {acc_pkg::CmdExecute,
  ...
  // Start ACC and then wait until done
  //
  // If the block gets reset, this task will exit early.
  protected task run_acc(input check_end_addr = 1);
    int exp_end_addr;
    int rd_pc;
    int wr_pc;

    // Check that we haven't been called re-entrantly. This could happen if there's a bug in the
    // reset sequence, which relies on run_acc() to exit properly when it sees a device reset.
    // Explode here if that happens, which should be easier to debug than two concurrent run_acc()
    ...
    // Wait for ACC to be idle. We expect getting there to take some time: ACC has to get two URND
    ...
    // Exit early if:
    // - ACC has been reset during the initial secure wipe; or
    // - ACC went from the initial secure wipe directly into the Locked state due to a fatal
    //   internal error (e.g., on URND).
    if (cfg.under_reset || cfg.model_agent_cfg.vif.status == acc_pkg::StatusLocked) begin
      running_ = 1'b0;
      return;
    end

    `DV_CHECK_FATAL(cfg.model_agent_cfg.vif.status == acc_pkg::StatusIdle,
                    "Timed out waiting for ACC to be idle before execution");

    fork : isolation_fork
      begin
        fork
          // Only _run_acc_cmd will complete
          _run_acc_cmd(acc_pkg::CmdExecute);
          _run_loop_warps();
        ...
        p_sequencer.acc_app_sequencer_h.stop_sequences();
    ...
    // The wait for ACC to finish short-circuits and exits immediately if the reset line goes low.
    ...
    if (check_end_addr && !absent_key_allowed) begin
      ...
      // it expects.
      exp_end_addr = AccMemUtilGetExpEndAddr(cfg.mem_util);
      if (exp_end_addr >= 0) begin
        `DV_CHECK_EQ_FATAL(exp_end_addr, cfg.model_agent_cfg.vif.stop_pc)
      end
    end

    // If ACC stopped with an error then it should trigger a recoverable or fatal alert. The
    ...
    if (cfg.model_agent_cfg.vif.status == acc_pkg::StatusLocked) begin
      bit cmd_wr;
      `DV_CHECK_STD_RANDOMIZE_WITH_FATAL(
      cmd_wr,
      cmd_wr dist {
    ...
    // synchronise with a "disable fork" in start_running_acc().
    running_ = 1'b0;
  endtask

  // Runs the transaction sequence for the ACC AppIntf
  ...
  // This is to allow for handshaking, and to prevent the task from terminating in run_acc.
  // Only the _run_acc_cmd fork is allowed to terminate in run_acc.
  virtual task _run_acc_app_rsp();
    forever begin
      acc_app_base_seq acc_app_seq;

      // Create and send sequence
      `uvm_create_on(acc_app_seq, p_sequencer.acc_app_sequencer_h);
      `DV_CHECK_RANDOMIZE_FATAL(acc_app_seq)
      `uvm_send(acc_app_seq)
    end
  endtask

  // Wait for ACC to finish, either by polling or by waiting on the interrupt pins
  //
      ...
      fork
        begin
          `DV_WAIT_TIMEOUT(10_000_000, ,
                           "Timed out waiting for ACC run to complete")
        end
        begin
          if (_pick_use_interrupt()) begin
            `uvm_info(`gfn, "\n\t ----| Waiting for ACC to finish (interrupt)", verbosity)
            wait_for_interrupt();
          end else begin
            uvm_reg_data_t run_status;

            `uvm_info(`gfn, "\n\t ----| Waiting for ACC to finish (polling)", verbosity)
            while (cfg.clk_rst_vif.rst_n) begin
              uvm_reg_data_t rdata;
              csr_rd(.ptr(ral.status), .value(rdata), .blocking(1), .backdoor(backdoor));
              if (rdata inside {acc_pkg::StatusIdle, acc_pkg::StatusLocked}) begin
                `uvm_info(`gfn, $sformatf("Seen end of ACC run (status is %0x)", rdata), UVM_HIGH)
                break;
              end
  ...
  // The guts of the run_acc task. Writes to the CMD register to start ACC and polls the status
  // register until completion. On reset, this returns immediately.
  protected task _run_acc_cmd(logic [7:0] cmd_i);
    case (cmd_i)
      acc_pkg::CmdSecWipeDmem: begin
        `uvm_info(`gfn, "\n\t ----| Starting ACC SecWipeDmem", UVM_MEDIUM)
      end
      acc_pkg::CmdSecWipeImem: begin
        `uvm_info(`gfn, "\n\t ----| Starting ACC SecWipeImem", UVM_MEDIUM)
      end
      acc_pkg::CmdExecute: begin
        `uvm_info(`gfn, "\n\t ----| Starting ACC Execution", UVM_MEDIUM)
      end
      default: `uvm_fatal(`gfn, "Invalid operation")
    endcase
    ...
    if (cfg.clk_rst_vif.rst_n) begin
      `uvm_info(`gfn, "\n\t ----| ACC finished", UVM_MEDIUM)
    end else begin
      `uvm_info(`gfn, "\n\t ----| ACC reset", UVM_MEDIUM)
    end
  endtask

  // How should we wait until ACC has finished?
  //
  // Use polling if ACC is not idle, the interrupt is disabled or if the interrupt pin is already
  ...
  protected function bit _pick_use_interrupt();
    ...
    // If ACC is not in the IDLE state, starting an operation won't actually do anything, and we
    // won't get a follow-up interrupt. So we shouldn't wait for an interrupt in this case!
    if (cfg.model_agent_cfg.vif.status != acc_pkg::StatusIdle) begin
      return 1'b0;
    end
  ...
  // forever, but is spawned by run_acc(), which will kill it when the ACC run completes or the
  // block is reset.
  protected task _run_loop_warps();
    ...
    forever begin
      ...
      // Do a DPI call to acc_memutil to look up whether there is a loop warp that we should be
      // taking. This returns 1'b1 and fills in new_count if there is a warp that does something.
      if (!AccMemUtilGetLoopWarp(cfg.mem_util, addr, old_count, new_count)) continue;
  ...
  protected function void _set_loop_counter(bit [31:0] loop_counter_idx, bit [31:0] new_count);
    ...
    counter_path = $sformatf("u_acc_loop_controller.g_loop_counters[%0d].u_loop_count",
    ...
    for (int k = 0; k < 2; k++) begin
      index = $sformatf("[%0d]", k);
      deposit_path_d =
          $sformatf({"tb.dut.u_acc_core.u_acc_controller.", counter_path, ".cnt_d", index});
      deposit_path_q =
          $sformatf({"tb.dut.u_acc_core.u_acc_controller.", counter_path, ".cnt_q", index});
  ...
  protected task _run_sideload_sequence();
    ...
    // sequence and then will be checked at the end of run_acc().
    ...
    if (absent_key_allowed) begin
      ...
      key_sideload_set_seq #(keymgr_pkg::acc_key_req_t) sideload_seq;
  ...
  virtual protected function string pick_elf_path();
    ...
    // Check that cfg.acc_elf_dir was set by the test
    `DV_CHECK_FATAL(cfg.acc_elf_dir.len() > 0);
    ...
    elf_path = AccTestHelperGetFilePath(helper, $urandom_range(num_files - 1));
    ...
    AccTestHelperFree(helper);

    return elf_path;
  endfunction

  // Start running ACC (forking off a run_acc task) and return when it's still going.
  //
  // This works by guessing the number of cycles to run and then returning after that time. Of
  // course, the run_acc task might actually finish in less time than that! If so, we run a second
  ...
  task start_running_acc(bit check_end_addr);
    bit timed_out = 1'b0;
    ...
    for (int i = 0; i < 10; i++) begin
      ...
      fork : isolation_fork
        begin
          fork
            run_acc(.check_end_addr(check_end_addr));
            begin
              wait(cfg.model_agent_cfg.vif.status == acc_pkg::StatusBusyExecute);
          ...
          // When we get here, we know that either the ACC sequence finished or we timed out and
          // it's still going. We can see whether ACC is still in the middle of a run by looking at
          ...
          if (cfg.model_agent_cfg.vif.status == acc_pkg::StatusBusyExecute) begin
            timed_out = 1'b1;
          end else begin
            timed_out = 1'b0;
            // The ACC sequence finished so update wait_cycles. cycle_counter should be at most equal
            // to wait_cycles because we'll stop at that point. It can be equal if ACC happens to
            // complete its operation in wait_cycles cycles.
            `DV_CHECK_LE_FATAL(cycle_counter, wait_cycles);
            longest_run_ = cycle_counter;

            // Wait for the run_acc thread to finish. This will usually be instant, but might take
            // a couple of cycles if we happen to have timed out exactly at the end of the run (when
            // the status has switched, but before run_acc finishes)
            wait(!running_);
    ...
    // We have stopped the run_acc process that was running. Update the "internal state to the
    // vseq" so that we can safely call this task another time. The running_ status is used to track
    // the fact that run_acc is going (not true because we just killed the process). Similarly,
  ...
  // ACC because it's not in sync with the logic that actually generates the alerts. We handle this
  // properly in the scoreboard so want to disable this check except in acc_common_vseq (which is
  ...
  // Task to check if acc is in locked state. If acc is indeed locked, then ensure fatal error is
  // asserted and reset the dut.
  virtual task reset_if_locked();
    uvm_reg_data_t act_val;
    wait (!(cfg.model_agent_cfg.vif.status inside {acc_pkg::StatusBusyExecute,
  ...
  protected task wait_edn_beat(edn_idx_e iface_idx, int max_wait_before);
    ...
    // wait up to max_wait_before cycles (main clock) for ACC to ask for some data.
    ...
    // Now we know that ACC is asking for *something*. Wait until the beat has completed.
  ...
  endtask
   ...
   // of them. The max_wait_before argument gives the number of cycles allowed before ACC asks for
   // each beat of data.
   //
