...
class acc_common_vseq extends acc_base_vseq;
  `uvm_object_utils(acc_common_vseq)
  bit sb_setting;
  ...
  task tl_access_w_abort(input bit [BUS_AW-1:0]    addr,
    ...
    if (completed && saw_err && !cfg.en_scb && tl_intg_err_type != TlIntgErrNone) begin
      `DV_WAIT(!(cfg.model_agent_cfg.vif.status inside {acc_pkg::StatusBusyExecute,
                                                     acc_pkg::StatusBusySecWipeInt}));
      `DV_CHECK_FATAL(ral.status.status.predict(acc_pkg::StatusLocked, .kind(UVM_PREDICT_READ)),
  ...
  task run_tl_intg_err_vseq(int num_times = 1);
    `DV_ASSERT_CTRL_REQ("acc_status_assert_en", 1'b0)
    super.run_tl_intg_err_vseq(num_times);
    `DV_ASSERT_CTRL_REQ("acc_status_assert_en", 1'b1)
  endtask
  ...
  virtual function void inject_intg_fault_in_passthru_mem(dv_base_mem mem,
    ...
    logic [otp_ctrl_pkg::AccKeyWidth-1:0]   key;
    logic [otp_ctrl_pkg::AccNonceWidth-1:0] nonce;
  ...
  virtual function void sec_cm_fi_ctrl_svas(sec_cm_base_if_proxy if_proxy, bit enable);
    if (enable) begin
      $asserton(0, "tb.dut.u_acc_core.u_acc_controller.ControllerStateValid");
      $asserton(0, "tb.MatchingStatus_A");
      $asserton(0, "tb.MatchingReqURND_A");
      $asserton(0, "tb.dut.u_acc_core.u_acc_start_stop_control.StartStopStateValid_A");
    end else begin
      $assertoff(0, "tb.dut.u_acc_core.u_acc_controller.ControllerStateValid");
      $assertoff(0, "tb.MatchingStatus_A");
      $assertoff(0, "tb.MatchingReqURND_A");
      $assertoff(0, "tb.dut.u_acc_core.u_acc_start_stop_control.StartStopStateValid_A");
    end
  ...
  virtual task sec_cm_inject_fault(sec_cm_base_if_proxy if_proxy);
    fork
      ...
      begin
        ...
        if (!uvm_re_match("*u_acc_start_stop_control*", if_proxy.path)) begin
