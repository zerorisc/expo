...
class acc_rf_base_intg_err_vseq extends acc_base_vseq;
  `uvm_object_utils(acc_rf_base_intg_err_vseq)
  ...
  // Start running a program through ACC and finish the task on a cycle when the selected register
  // file is being used.
  task run_until_use();
    int num_tries = 4;
    ...
    repeat (num_tries) begin
      ...
      // Start running ACC. When this task returns, we'll be in the middle of a run.
      ...
      // The program run has completed. Before we try again, we need to allow ACC to finish its
    ...
    // side. This shouldn't really happen, because it means that start_running_acc() picked a time
  ...
  function bit [acc_pkg::BaseIntgWidth-1:0] corrupt_data(
      input bit [acc_pkg::BaseIntgWidth-1:0] orig_data
    );
    bit [acc_pkg::BaseIntgWidth-1:0] mask;
    `DV_CHECK_STD_RANDOMIZE_WITH_FATAL(mask, $countones(mask) inside {[1:2]};)
    return cfg.fix_integrity_32(orig_data) ^ mask;
  endfunction

  task inject_errors();
    logic [acc_pkg::BaseIntgWidth-1:0] orig_data;
    bit [acc_pkg::BaseIntgWidth-1:0] new_data;
  ...
  task body();
    ...
    acc_pkg::err_bits_t err_bits;

    elf_path = pick_elf_path();
    `uvm_info(`gfn, $sformatf("Loading ACC binary from `%0s'", elf_path), UVM_LOW)
    load_elf(elf_path, 1'b1);

    // Start running ACC. When this task returns, we'll be in the middle of a run and executing an
    ...
    // ACC should now do a secure wipe
    wait_secure_wipe();

    // We should now be in a locked state after the secure wipe.
    `DV_CHECK_FATAL(cfg.model_agent_cfg.vif.status == acc_pkg::StatusLocked);
