...
class acc_partial_wipe_vseq extends acc_single_vseq;
  `uvm_object_utils(acc_partial_wipe_vseq)
  `uvm_object_new
  ...
  task inject_partial_wipe();
    string hdl_path = "tb.dut.u_acc_core";
    int    ticks_until_err = 1;
    randcase
      1: hdl_path = {hdl_path, ".u_acc_alu_bignum.sec_wipe_mod_urnd_i"};
      1: hdl_path = {hdl_path, ".u_acc_controller.sec_wipe_zero_i"};
      1: hdl_path = {hdl_path, ".sec_wipe_base"};
      1: hdl_path = {hdl_path, ".u_acc_rf_base.sec_wipe_stack_reset_i"};
      1: begin
    ...
    `DV_WAIT(cfg.model_agent_cfg.vif.status != acc_pkg::StatusBusySecWipeInt)
  ...
  task dut_init(string reset_kind = "HARD");
    ...
    // Override the do_end_addr_check flag. It is set in acc_single_vseq, but we might cause a stop
