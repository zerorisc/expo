...
class acc_sequential_vseq extends acc_base_vseq;
  `uvm_object_utils(acc_sequential_vseq)
  ...
  protected function string_queue_t list_elf_paths();
    ...
    `DV_CHECK_FATAL(num_files > 0,
                    $sformatf("No regular files found in directory `%0s'.", cfg.acc_elf_dir))

    // Get each path from the C++ code. AccTestHelperGetFilePath returns the empty string on error.
    for (int idx = 0; idx < num_files; idx++) begin
      buffer = AccTestHelperGetFilePath(helper, idx);
      `DV_CHECK_FATAL(buffer.len() > 0, "Bad index for ELF file")

      // Take a copy of the string so that we can safely call AccTestHelperGetFilePath on the next
  ...
  task body();
    ...
    foreach (paths[i]) begin
      `uvm_info(`gfn, $sformatf("Loading ACC binary from `%0s'", paths[i]), UVM_LOW)
