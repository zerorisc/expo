...
// A sequence that runs a program multiple times and corrupts an integrity-checked signal while ACC
// is still running.

class acc_intg_err_vseq extends acc_base_vseq;

  `uvm_object_utils(acc_intg_err_vseq)
  ...
  protected virtual task await_use(output bit [acc_pkg::BaseWordsPerWLEN-1:0] used_words);
  endtask
  ...
  // were corrupted.
  protected function bit [acc_pkg::ExtWLEN-1:0] corrupt_data(
      input bit [acc_pkg::ExtWLEN-1:0] orig_data,
      input int unsigned corrupt_word_pct[acc_pkg::BaseWordsPerWLEN],
      output bit [acc_pkg::BaseWordsPerWLEN-1:0] corrupted_words
    );
    bit [acc_pkg::ExtWLEN-1:0] new_data;
    for (int i_word = 0; i_word < acc_pkg::BaseWordsPerWLEN; i_word++) begin
  ...
  protected virtual task inject_errors(input  bit [acc_pkg::BaseWordsPerWLEN-1:0] used_words,
                                       output bit [acc_pkg::BaseWordsPerWLEN-1:0] corrupted_words);
  endtask
  ...
  task body();
    ...
    if (|(corrupted_words & used_words)) begin
      // ACC should now do a secure wipe. Give it up to 4000 cycles to do so (because it needs to
