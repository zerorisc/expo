...
class acc_reset_vseq extends acc_base_vseq;
  `uvm_object_utils(acc_reset_vseq)
  ...
  task body();
    ...
    for (int i = 0; i < num_iters; i++) begin
      ...
      // Start ACC. When the task returns, we'll be part-way through a run.
      start_running_acc(.check_end_addr(1'b1));
