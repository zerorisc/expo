...
// A sequence to verify the countermeasure(s) ACC.*_STACK.ADDR.INTEGRITY.

class acc_stack_addr_integ_chk_vseq extends acc_single_vseq;
  `uvm_object_utils(acc_stack_addr_integ_chk_vseq)
  `uvm_object_new
  ...
  // Wait until the value at path is nonzero or until ACC finishes execution.
  //
  ...
  task wait_for_flag(string path);
    fork begin : isolation_fork
      fork
        ...
        // This process waits until ACC completes execution or until reset. Passing backdoor=1
  ...
  task inject_integ_err();
    ...
    if (err_type) begin
      ...
      base_path = "tb.dut.u_acc_core.u_acc_rf_base";
      stack_path = {base_path, ".u_call_stack"};
      msb = 38;
    end else begin
      // We want to inject an error into the loop stack
      base_path = "tb.dut.u_acc_core.u_acc_controller.u_acc_loop_controller";
  ...
  task corrupt_stack(string       stack_path,
    ...
    if (immediately_escalate) begin
      ...
      // will be visible through the top_data_o port from acc_stack. This is almost immediate, but
...
endclass : acc_stack_addr_integ_chk_vseq
