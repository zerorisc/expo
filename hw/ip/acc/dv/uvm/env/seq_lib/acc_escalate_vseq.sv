...
class acc_escalate_vseq extends acc_base_vseq;
  `uvm_object_utils(acc_escalate_vseq)
  ...
  task body();
    ...
    acc_pkg::acc_start_stop_state_e start_stop_state_for_escalate;

    elf_path = pick_elf_path();
    `uvm_info(`gfn, $sformatf("Loading ACC binary from `%0s'", elf_path), UVM_LOW)

    // Pick whether we're going to send RMA request or escalation to ACC.
    select_rma_req = $urandom_range(1);

    // Choose when we send the lc_ctrl stimulus
    // Immediate - Within 5 cycles of simulation start and before we run the ACC program
    // Before - Within 100 cycles of simulation start and before we run the ACC program
    // During - During the ACC run
    // Sec Wipe State - When we observe a particular state of the secure wipe process
    // After - After the ACC run is complete
    ...
    if (escalate_timing == GO_AFTER) begin
      // We want to send the escalation/RMA signal after ACC is finished. Run an operation, send
    ...
    if (escalate_timing == GO_DURING) begin
      // Start ACC running. When this task returns, we'll be in the middle of a run.
      ...
      `DV_CHECK_STD_RANDOMIZE_WITH_FATAL(start_stop_state_for_escalate,
        start_stop_state_for_escalate inside {
          acc_pkg::AccStartStopSecureWipeWdrUrnd,
          acc_pkg::AccStartStopSecureWipeAccModBaseUrnd,
          acc_pkg::AccStartStopSecureWipeAllZero,
          acc_pkg::AccStartStopSecureWipeComplete
        };
      )
      // Start running ACC and wait for the state to reach the chosen one in parallel. Fork exits
      // only once we've hit the chosen state
      fork
        start_running_acc(.check_end_addr(1'b0));
        wait (cfg.trace_vif.acc_start_stop_state == start_stop_state_for_escalate);
      join
    end

    // Send an escalation/RMA signal immediately (the randomisation about where we should strike
    // has already been done inside start_running_acc() for GO_DURING)
  ...
  task send_rma_request_signal(int unsigned max_cycles);
    ...
    `DV_SPINWAIT_EXIT(
      ...
      "Not getting rma_ack from ACC for 500 cycles!")
  endtask
