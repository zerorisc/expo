...
// ACC is trying to read from it.

class acc_rf_bignum_intg_err_vseq extends acc_intg_err_vseq;
  `uvm_object_utils(acc_rf_bignum_intg_err_vseq)
  ...
  protected task await_use(output bit [acc_pkg::BaseWordsPerWLEN-1:0] used_words);
    ...
    `DV_SPINWAIT_EXIT(
      ...
      "Not getting selected rd_en from ACC for 2000 cycles!"
    )
  endtask
  ...
  protected task inject_errors(input  bit [acc_pkg::BaseWordsPerWLEN-1:0] used_words,
    ...
    logic [acc_pkg::ExtWLEN-1:0] orig_data;
