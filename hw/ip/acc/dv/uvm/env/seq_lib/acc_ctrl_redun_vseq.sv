...
class acc_ctrl_redun_vseq extends acc_single_vseq;
  `uvm_object_utils(acc_ctrl_redun_vseq)
  `uvm_object_new
  ...
  task body();
    ...
    // Try to run an ACC application to completion, injecting an error in the middle. We might be
  ...
  // Run the acc_single_vseq body, which runs a single ACC application to completion. At the
  // same time, try to inject an error and check that the RTL (and model) both spot the error.
  task try_once();
    fork begin : isolation_fork
      fork
        // Run the acc_single_vseq body
        super.body();
      ...
      // which lets the run_acc task tidy up properly.
  ...
  task inject_redun_err();
    ...
    case(err_type)
      0: begin
        ...
        err_path = "tb.dut.u_acc_core.u_acc_alu_bignum.ispr_addr_i";
        wait_for_flag("tb.dut.u_acc_core.u_acc_alu_bignum.ispr_wr_en", 1);
      ...
      1: begin
        ...
        err_path = "tb.dut.u_acc_core.u_acc_alu_bignum.ispr_addr_i";
        wait_for_flag("tb.dut.u_acc_core.u_acc_alu_bignum.ispr_rd_en_i", 1);
        `DV_CHECK_FATAL(uvm_hdl_read(err_path, good_addr));
        // Mask to corrupt 1 to 2 bits of the ispr addr
        `DV_CHECK_STD_RANDOMIZE_WITH_FATAL(mask, $countones(mask) inside {[1:2]};)
        bad_addr = good_addr ^ mask;
      ...
      2: begin
        ...
        string alu_path = "tb.dut.u_acc_core.u_acc_alu_bignum";
        ...
        err_path = "tb.dut.u_acc_core.u_acc_alu_bignum.operation_i.op";
        ...
        while (1) begin
          ...
          if (good_op <= acc_pkg::AluOpBignumNot)
            break;
        ...
        if (!selected_flags_C) begin
          if (good_op inside {acc_pkg::AluOpBignumAdd, acc_pkg::AluOpBignumAddc,
                              acc_pkg::AluOpBignumSub, acc_pkg::AluOpBignumSubb}) begin
            avoid_addc = 1'b1;
          end
          if (good_op inside {acc_pkg::AluOpBignumAddm, acc_pkg::AluOpBignumAddv,
                              acc_pkg::AluOpBignumSubm, acc_pkg::AluOpBignumSubv,
                              acc_pkg::AluOpBignumAddvm, acc_pkg::AluOpBignumSubvm}) begin
            avoid_addv = 1'b1;
          end
        end
        ...
        `DV_CHECK_STD_RANDOMIZE_WITH_FATAL(bad_op,
                                           bad_op != good_op;
                                           bad_op != acc_pkg::AluOpBignumNone;
                                           avoid_addc -> (bad_op != acc_pkg::AluOpBignumAdd &&
                                                          bad_op != acc_pkg::AluOpBignumAddc &&
                                                          bad_op != acc_pkg::AluOpBignumSub &&
                                                          bad_op != acc_pkg::AluOpBignumSubb);
                                           bad_op != good_op + 3; good_op != bad_op + 3;)

        `DV_CHECK_STD_RANDOMIZE_WITH_FATAL(bad_op,
                                           bad_op != good_op;
                                           bad_op != acc_pkg::AluOpBignumNone;
                                           avoid_addv -> (bad_op != acc_pkg::AluOpBignumAddm &&
                                                          bad_op != acc_pkg::AluOpBignumAddv &&
                                                          bad_op != acc_pkg::AluOpBignumSubm &&
                                                          bad_op != acc_pkg::AluOpBignumSubv &&
                                                          bad_op != acc_pkg::AluOpBignumAddvm &&
                                                          bad_op != acc_pkg::AluOpBignumSubvm);
      ...
      3: begin
        bit choose_err;
        acc_pkg::insn_dec_shared_t insn_dec_shared_i;
        `DV_CHECK_STD_RANDOMIZE_FATAL(choose_err)

        report_err_type($sformatf("error on lsu_addr_en (choose_err = %0d)", choose_err));

        err_path = "tb.dut.u_acc_core.u_acc_controller.insn_dec_shared_i";
        wait_for_flag("tb.dut.u_acc_core.u_acc_controller.insn_valid_i", 1);
      ...
      4: begin
        ...
        wait_for_flag("tb.dut.u_acc_core.insn_valid", 1);
        case(choose_err)
          0: begin
            bit [31:0] bad_rf_ren_a;
            bit [31:0] good_rf_ren_a;
            err_path = "tb.dut.u_acc_core.rf_predec_bignum.rf_ren_a";
          ...
          1: begin
            ...
            err_path = "tb.dut.u_acc_core.rf_predec_bignum.rf_ren_b";
          ...
          2: begin
            ...
            err_path = "tb.dut.u_acc_core.ispr_predec_bignum.ispr_rd_en";
      ...
      5: begin
        ...
        report_err_type($sformatf("error in acc_mac_bignum (choose_err=%0d, after %0d clocks)",
                                  choose_err, num_clks));

        `DV_WAIT(cfg.model_agent_cfg.vif.status == acc_pkg::StatusBusyExecute)
        cfg.clk_rst_vif.wait_clks(num_clks);
        // Wait for valid instruction, because `acc_core` only propagates bignum MAC predec errors
        // for valid instructions.
        wait_for_flag("tb.dut.u_acc_core.insn_valid", 1);
        case(choose_err)
          0: begin
            err_path = "tb.dut.u_acc_core.u_acc_mac_bignum.mac_en_i";
            `DV_CHECK_FATAL(uvm_hdl_read(err_path, mac_en));
            `DV_CHECK_FATAL(uvm_hdl_force(err_path, !mac_en) == 1);
          end
          1: begin
            bit zero_acc;
            err_path = "tb.dut.u_acc_core.u_acc_mac_bignum.operation_i.zero_acc";
            wait_for_flag("tb.dut.u_acc_core.u_acc_mac_bignum.mac_en_i", 1);
      ...
      6: begin
        ...
        report_err_type($sformatf("error in acc_rf_bignum (choose_err=%0d)", choose_err));
        case(choose_err)
          0: begin
            err_path = "tb.dut.u_acc_core.u_acc_rf_bignum.wr_addr_i[4:0]";
            wait_for_flag("tb.dut.u_acc_core.u_acc_rf_bignum.wr_en_i[1:0]", 2);
          ...
          1: begin
            err_path = "tb.dut.u_acc_core.u_acc_rf_bignum.rd_addr_a_i";
            wait_for_flag("tb.dut.u_acc_core.u_acc_rf_bignum.rd_en_a_i", 1);
            `DV_CHECK_FATAL(uvm_hdl_read(err_path, addr));
            `DV_CHECK_STD_RANDOMIZE_WITH_FATAL(mask, $countones(mask) inside {[1:2]};)
            addr = addr ^ mask;
            `DV_CHECK_FATAL(uvm_hdl_force(err_path, addr) == 1);
          ...
          2: begin
            err_path = "tb.dut.u_acc_core.u_acc_rf_bignum.rd_addr_b_i";
            wait_for_flag("tb.dut.u_acc_core.u_acc_rf_bignum.rd_en_b_i", 1);
            `DV_CHECK_FATAL(uvm_hdl_read(err_path, addr));
            `DV_CHECK_STD_RANDOMIZE_WITH_FATAL(mask, $countones(mask) inside {[1:2]};)
            addr = addr ^ mask;
            `DV_CHECK_FATAL(uvm_hdl_force(err_path, addr) == 1);
