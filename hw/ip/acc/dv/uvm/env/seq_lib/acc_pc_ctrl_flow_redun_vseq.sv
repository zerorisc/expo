...
class acc_pc_ctrl_flow_redun_vseq extends acc_single_vseq;
  `uvm_object_utils(acc_pc_ctrl_flow_redun_vseq)
  `uvm_object_new
  ...
  task corrupt_prefetch_addr();
    ...
    string addr_path = "tb.dut.u_acc_core.u_acc_instruction_fetch.insn_prefetch_addr";

    do begin
      @(cfg.clk_rst_vif.cb);
      if (!uvm_hdl_read("tb.dut.u_acc_core.u_acc_instruction_fetch.imem_rvalid_i", imem_rvalid))
        `uvm_fatal(`gfn, "failed to read imem_rvalid_i");
      if (!uvm_hdl_read("tb.dut.u_acc_core.u_acc_instruction_fetch.insn_fetch_req_valid_raw_i",
                        insn_fetch_req_valid))
        `uvm_fatal(`gfn, "failed to read insn_fetch_req_valid_raw_i");
      if (!uvm_hdl_read("tb.dut.u_acc_core.u_acc_instruction_fetch.prefetch_ignore_errs_i",
                        prefetch_ignore_err))
...
endclass : acc_pc_ctrl_flow_redun_vseq
