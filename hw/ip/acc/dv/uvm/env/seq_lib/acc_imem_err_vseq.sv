...
class acc_imem_err_vseq extends acc_base_vseq;
  `uvm_object_utils(acc_imem_err_vseq)
  ...
  task body();
    ...
    // If we were unlucky, we might have injected the errors while ACC was executing an instruction
    ...
    // If ACC is now idle, we hit this exact window and the test didn't do anything useful. Ho
