...
// The main skeleton is acc_multi test in which we run multiple programs. It is better suited
// to see RND behaviour between different ACC operations.
...
class acc_rnd_sec_cm_vseq extends acc_multi_vseq;
  `uvm_object_utils(acc_rnd_sec_cm_vseq)
  `uvm_object_new
  ...
  virtual task _run_rnd_edn_rsp();
    forever begin
      ...
      if(cfg.poll_rnd_edn_req() && ~cfg.rnd_vif.edn_rnd_ack_i) begin
        ...
        // case.
          repeat (acc_pkg::EdnDataWidth/cip_base_pkg::EDN_BUS_WIDTH - 1)
