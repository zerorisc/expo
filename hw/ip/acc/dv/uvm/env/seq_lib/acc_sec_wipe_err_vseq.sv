...
class acc_sec_wipe_err_vseq extends acc_base_vseq;
  `uvm_object_utils(acc_sec_wipe_err_vseq)
  ...
  task send_spurious_req();
    ...
    // Secure wipe requests are not allowed when ACC is idle; so wait for ACC to become idle.
    ...
    `uvm_info(`gfn, "Requesting secure wipe while ACC is idle, which is not allowed.", UVM_LOW)
  ...
  task drop_wipe_request();
    ...
    // Wait until binary has completed execution and ACC does the internal secure wipe.
    `uvm_info(`gfn, "Waiting for ACC to complete execution.", UVM_LOW)
    `DV_WAIT(cfg.model_agent_cfg.vif.status != acc_pkg::StatusBusyExecute)
    if (cfg.model_agent_cfg.vif.status != acc_pkg::StatusBusySecWipeInt) begin
      // If ACC is no longer executing but also not performing the internal secure wipe, we have
      // missed the opportunity (the `start_running_acc()` function does not guarantee that it
      // returns while ACC is still running). So we cannot inject the error anymore.
  ...
  task send_spurious_ack();
    ...
    if (while_executing) begin
      // Secure wipe acknowledges are not allowed when ACC is executing; so load and execute a
      // binary.
      string elf_path = pick_elf_path();
      load_elf(elf_path, 1'b1);
      `uvm_info(`gfn, $sformatf("Executing ACC binary `%0s'.", elf_path), UVM_LOW)
      ...
      // If we are unlucky, ACC is now already securely wiping due to a different error.  In
      // that case, we skip error injection.
      if (cfg.model_agent_cfg.vif.status == acc_pkg::StatusBusySecWipeInt) begin
        skip_err_injection = 1;
      end
    end else begin
      // Secure wipe acknowledges are not allowed when ACC is idle; so wait for ACC to become
      // idle.
      `DV_WAIT(cfg.model_agent_cfg.vif.status == acc_pkg::StatusIdle)
      @(cfg.clk_rst_vif.cbn);
    end
