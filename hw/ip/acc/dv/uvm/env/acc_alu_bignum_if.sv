...
//
// Bound into the acc_alu_bignum and used to help collect ISPR information for coverage.

interface acc_alu_bignum_if (
  input         clk_i,
  input         rst_ni,

  input [acc_pkg::ExtWLEN-1:0] mod_intg_q,
  input                         mod_used
);
  ...
  function static void force_mod_intg_q(input logic [acc_pkg::ExtWLEN-1:0] should_val);
    force u_acc_alu_bignum.mod_intg_q = should_val;
  endfunction

  // Release the forcing of the `mod_intg_q` register.
  function automatic void release_mod_intg_q();
    release u_acc_alu_bignum.mod_intg_q;
  endfunction
