...
//
// The monitor watches the acc_model_if and reports responses from the model
//

class acc_model_monitor extends dv_base_monitor #(
    .ITEM_T (acc_model_item),
    .CFG_T  (acc_model_agent_cfg)
  );
  `uvm_component_utils(acc_model_monitor)

  // the base class provides the following handles for use:
  // acc_model_agent_cfg: cfg
  // acc_model_agent_cov: cov
  // uvm_analysis_port #(acc_model_item): analysis_port
  ...
  protected task collect_status();
    acc_model_item trans;

    forever begin
      trans = acc_model_item::type_id::create("trans");
      ...
      trans.item_type = AccModelStatus;
  ...
  protected task collect_insns();
    ...
    // AccModelInsn items are only used for coverage collection; disable their detection if
    ...
    forever begin
      // Use a clocking block to ensure we sample after the always_ff code in acc_core_model.sv has
      ...
      if (cfg.vif.rst_ni === 1'b1) begin
        ...
        if (acc_trace_checker_pop_iss_insn(insn_addr, insn_mnemonic)) begin
          trans = acc_model_item::type_id::create("trans");
          trans.item_type = AccModelInsn;
          trans.status    = 0;
          trans.err       = 0;
