...
// of the acc_core_model module. For example, you might instantiate it in your testbench next to
...
#(
  ...
  //
  // Call this on a negedge of clk_i to ensure sequencing with the acc_model_step on the following
  // posedge.
  function automatic void invalidate_imem();
    `uvm_info("acc_model_if", "Invalidating IMEM", UVM_HIGH)
    `DV_CHECK_FATAL(u_model.acc_model_invalidate_imem(handle) == 0,
                    "Failed to invalidate IMEM", "acc_model_if")
  endfunction
  ...
  function automatic void invalidate_dmem();
    `uvm_info("acc_model_if", "Invalidating DMEM", UVM_HIGH)
    `DV_CHECK_FATAL(u_model.acc_model_invalidate_dmem(handle) == 0,
                    "Failed to invalidate DMEM", "acc_model_if")
  endfunction
  ...
  function automatic bit [31:0] step_crc(bit [47:0] item, bit [31:0] crc_state);
    `DV_CHECK_FATAL(u_model.acc_model_step_crc(handle, item, crc_state) == 0,
                    "Failed to update CRC", "acc_model_if")
    return crc_state;
  endfunction

  // Pass loop warp rules to the model
  function automatic void take_loop_warps(chandle memutil);
    u_model.acc_take_loop_warps(handle, memutil);
  endfunction

  function automatic bit has_loop_warps(chandle memutil);
    return u_model.acc_has_loop_warps(memutil) != 0;
  endfunction
  ...
  task automatic send_err_escalation(bit [31:0] err_val);
    ...
    `DV_CHECK_FATAL(u_model.acc_model_send_err_escalation(handle, err_val, 1'b0) == 0,
                    "Failed to escalate errors", "acc_model_if")
  ...
  task automatic lock_immediately(bit [31:0] err_val);
    ...
    `uvm_info("acc_model_if", "Locking immediately after error", UVM_HIGH)
    force u_model.wakeup_iss = 1;
    `DV_CHECK_FATAL(u_model.acc_model_send_err_escalation(handle, err_val, 1'b1) == 0,
  ...
  function automatic void set_software_errs_fatal(bit new_val);
    `uvm_info("acc_model_if", "writing to software_errs_fatal", UVM_HIGH);
    `DV_CHECK_FATAL(u_model.acc_model_set_software_errs_fatal(handle, new_val) == 0,
                    "Failed to set software_errs_fatal", "acc_model_if")
  endfunction

  function automatic void acc_set_no_sec_wipe_chk();
    `uvm_info("acc_model_if", "writing to no_sec_wipe_data_chk", UVM_HIGH);
    `DV_CHECK_FATAL(u_model.acc_set_no_sec_wipe_chk(handle) == 0,
                    "Failed to set no_sec_wipe_data_chk", "acc_model_if")
  endfunction

  function automatic void acc_disable_stack_check();
    `uvm_info("acc_model_if", "Disabling stack integrity checks", UVM_HIGH);
    `DV_CHECK_FATAL(u_model.acc_disable_stack_check(handle) == 0,
                    "Failed to disable stack integrity checks", "acc_model_if")
  endfunction
