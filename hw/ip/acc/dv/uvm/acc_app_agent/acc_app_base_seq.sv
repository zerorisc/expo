...
class acc_app_base_seq extends dv_base_seq #(
    .REQ         (acc_app_item),
    .RSP         (acc_app_item),
  ...
  `uvm_object_utils(acc_app_base_seq)
  `uvm_object_new
  ...
  virtual task body();
    ...
    while (!finish_seq) begin
      ...
      if (!item.req_hold && !saw_first_rsp) begin
        acc_app_item rsp = acc_app_item::type_id::create("rsp_item");
        rsp.rsp_ready         = 0;
  ...
  task automatic compute_dpi_digest (
    ...
    case (hash_mode)
      ...
      sha3_pkg::Sha3: begin
        case (strength)
          sha3_pkg::L224: begin
            `uvm_fatal(`gfn, $sformatf("strength[%0s] is not allowed for ACC sha3", strength.name()))
          end
          sha3_pkg::L256: begin
      ...
      sha3_pkg::CShake: begin
        `uvm_fatal(`gfn, $sformatf("mode[%0s] is not allowed for ACC", hash_mode.name()))
      end
    endcase
  endtask
