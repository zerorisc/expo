# ACC Random Instruction Generator

This directory contains a random instruction generator for ACC called
`acc-rig`. This is intended to be run in multiple phases. If you just
...
Example usage:
```
hw/ip/acc/dv/rig/acc-rig gen --seed 123 --size 1000 >foo.json
```
...
assembled and linked using the toolchain in `hw/ip/acc/util`.
...
Example usage:
```
hw/ip/acc/dv/rig/acc-rig asm --output foo foo.json
```
...
hw/ip/acc/util/acc_ld.py -o foo.elf -T foo.ld foo.o
```
