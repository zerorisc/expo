# ACC Random Instruction Generator - Internals
...
The generated program then gets used in `acc-rig` (which lives in the parent directory) to create proper output files to simulate the generated instructions.
...
In order to generate concise programs that don't run into architectural problems RIG needs to have an idea about how ACC works, what's a memory, a register etc.
