...
module acc_top_coco #(
  ...
  reg                      acc_start;
  // Intialise acc_start_done to 1 so that we only signal acc_start after we have seen a reset. If
  // you don't do this, we start ACC before the reset, which can generate confusing trace messages.
  reg                      acc_start_done = 1'b1;
  ...
  // ACC cycle counter to easily inspect waves and associate those waves with COCO-ALMA tool
  // output. The counter is reset with the start pulse being sent.
  reg [31:0] acc_cycle_cnt_q;
  always @(posedge clk_sys or negedge rst_sys_n) begin
    if (!rst_sys_n) begin
      acc_cycle_cnt_q <= 32'b0;
    end else begin
      if (acc_start) begin
        acc_cycle_cnt_q <= 32'b0;
      end else if (acc_start_done) begin
        acc_cycle_cnt_q <= acc_cycle_cnt_q + 1;
      end
    end
  end
  assign acc_cycle_cnt_o = acc_cycle_cnt_q;
