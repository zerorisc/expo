...
//! -----
//! ```rust
//!     let _mux_acc = crate::acc::AccelMuxComponent::new(&peripherals.acc)
//!         .finalize(acc_mux_component_static!());
//!
//!     peripherals.acc.initialise(
//!         dynamic_deferred_caller
//!             .register(&peripherals.acc)
...
use lowrisc::acc::Acc;
use lowrisc::virtual_acc::{MuxAccel, VirtualMuxAccel};

#[macro_export]
macro_rules! acc_mux_component_static {
    () => {{ kernel::static_buf!(lowrisc::virtual_acc::MuxAccel<'static>) }};
}

#[macro_export]
macro_rules! acc_component_static {
    () => {{ kernel::static_buf!(lowrisc::virtual_acc::VirtualMuxAccel<'static>) }};
}

pub struct AccelMuxComponent {
    acc: &'static Acc<'static>,
}

impl AccelMuxComponent {
    pub fn new(acc: &'static Acc<'static>) -> AccelMuxComponent {
        AccelMuxComponent { acc }
    }
}
...
impl Component for AccelMuxComponent {
    ...
    fn finalize(self, s: Self::StaticInput) -> Self::Output {
        s.write(MuxAccel::new(self.acc))
    }
}

pub struct AccComponent {
    mux_acc: &'static MuxAccel<'static>,
}

impl AccComponent {
    pub fn new(mux_acc: &'static MuxAccel<'static>) -> AccComponent {
        AccComponent { mux_acc }
    }
}

impl Component for AccComponent {
    type StaticInput = &'static mut MaybeUninit<VirtualMuxAccel<'static>>;

    type Output = &'static VirtualMuxAccel<'static>;

    fn finalize(self, s: Self::StaticInput) -> Self::Output {
        let virtual_acc_user = s.write(VirtualMuxAccel::new(self.mux_acc));

        virtual_acc_user
    }
}

/// Find the ACC app in the Tock process list
///
...
///    * ACC imem start address
///    * ACC imem size
///    * ACC dmem start address
///    * ACC dmem size
///
