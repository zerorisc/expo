...
/**
 * This test runs every instruction in ACC's ISA and checks the result.
 *
 * It reuses the `smoke_test.s` script used in ACC's Design Verification,
...
bool test_main(void) {
  ...
  CHECK(kExpectedInstrCount == instruction_count,
        "Expected ACC to execute %d instructions, but it exected %d",
