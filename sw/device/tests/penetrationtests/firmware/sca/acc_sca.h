...
#ifndef OPENTITAN_SW_DEVICE_TESTS_PENETRATIONTESTS_FIRMWARE_SCA_ACC_SCA_H_
#define OPENTITAN_SW_DEVICE_TESTS_PENETRATIONTESTS_FIRMWARE_SCA_ACC_SCA_H_
 ...
 * for each key the key generation operation on ACC is started.
 *
 ...
 * for each seed the key generation operation on ACC is started.
 *
 * @param uj An initialized uJSON context.
 * @return OK or error.
 */
...
/**
 * acc.sca.ecdsa256.sign command handler.
 *
...
/**
 * acc.sca.ecdsa256.sign_batch command handler.
 *
 * Same as acc.sca.ecdsa256.sign but in batch mode. Random message, random
...
/**
 * acc.sca.ecdsa256.sign_fvsr_batch command handler.
 *
 * Same as acc.sca.ecdsa256.sign but in batch mode. Fixed or random message,
...
/**
 * Initializes the ACC SCA test on the device.
 *
 * @param uj An initialized uJSON context.
 * @return OK or error.
 */
...
/**
 * Initializes the Keymanager used for the ACC SCA tests.
 *
 * @param uj An initialized uJSON context.
 * @return OK or error.
 */
...
/**
 * acc.sca.insn.carry_flag command handler.
 *
 * Receive big_num from host. On ACC, add big_num + big_num and get the
...
/**
 * Command handler for the acc.sca.key_sideload_fvsr test.
 *
 * Side-load 16 fixed vs. random keys from keymanager to ACC.
 *
 * @param uj An initialized uJSON context.
 * @return OK or error.
 */
...
/**
 * Command handler for the acc.sca.rsa512_decrypt test.
 *
...
/**
 * Command handler for the acc.sca.combi_ops test.
 *
 * Loads two fixed values to the ACC and performs several operations.
 ...
 *
 * A second trigger window can be chosen which is over the wiping of the ACC.
 *
...
/**
 * ACC SCA command handler.
 *
 * Command handler for the ACC SCA command.
 *
 * @param uj An initialized uJSON context.
 * @return OK or error.
 */
...
#endif  // OPENTITAN_SW_DEVICE_TESTS_PENETRATIONTESTS_FIRMWARE_SCA_ACC_SCA_H_
