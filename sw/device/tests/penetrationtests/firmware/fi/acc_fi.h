...
#ifndef OPENTITAN_SW_DEVICE_TESTS_PENETRATIONTESTS_FIRMWARE_FI_ACC_FI_H_
#define OPENTITAN_SW_DEVICE_TESTS_PENETRATIONTESTS_FIRMWARE_FI_ACC_FI_H_

#include "sw/device/lib/base/status.h"
#include "sw/device/lib/ujson/ujson.h"

/**
 * acc.fi.char.beq command handler.
 *
...
/**
 * acc.fi.char.bn_rshi command handler.
 *
...
/**
 * acc.fi.char.bn_sel command handler.
 *
...
/**
 * acc.fi.char.bn_wsrr command handler.
 *
...
/**
 * acc.fi.char.bne command handler.
 *
...
/**
 * acc.fi.char_dmem_access command handler.
 *
 * ACC loads WDRs with words from DMEM. These values are stored in different
 * data sections.
 *
 * Faults are injected during the trigger_high & trigger_low.
 *
...
/**
 * acc.fi.char_dmem_write command handler.
 *
...
/**
 * acc.fi.char.hardware.dmem.op.loop command handler.
 *
 * This FI penetration tests executes the following instructions on ACC:
...
/**
 * acc.fi.char.hardware.reg.op.loop command handler.
 *
 * This FI penetration tests executes the following instructions on ACC:
 * - Initialize register x3=0
 * - Perform 10000 x3 = x3 + 1 additions using hardware loop instructions
...
/**
 * acc.fi.char.jal command handler.
 *
...
/**
 * acc.fi.char.lw command handler.
 *
...
/**
 * acc.fi.char_mem command handler.
 *
 * Initializes IMEM and DMEM of ACC with a fixed pattern. Inject a fault and
...
/**
 * acc.fi.char_rf command handler.
 *
 * Init GPRs and WDRs of ACC with reference values. Inject faults during 10000
...
/**
 * acc.fi.char.unrolled.dmem.op.loop command handler.
 *
 * This FI penetration tests executes the following instructions on ACC:
 * - Perform 100 times:
...
/**
 * acc.char.unrolled.reg.op.loop command handler.
 *
 * This FI penetration tests executes the following instructions on ACC:
 ...
 * @return OK or error.
 */
status_t handle_acc_init(ujson_t *uj);
...
/**
 * acc.fi.key_sideload command handler.
 *
 * Injects a fault when a key is sideloaded from the key manager into ACC.
 *
 * Faults are injected during the trigger_high & trigger_low.
 *
 * @param uj An initialized uJSON context.
...
/**
 * acc.fi.load_integrity command handler.
 *
 * Tests, whether a fault during loading the ACC app can manipulate data in
 * DMEM without changing the CRC-32 checksum that is used to check the
 * integrity of the DMEM and IMEM.
 *
 * As the ACC app itself is not the target of this FI analysis, it only
...
/**
 * acc.fi.pc command handler.
 *
 * The goal of this test is to fault the ACC program counter that is passed
 * from Ibex into ACC. ACC jumps to this PC. Manipulation can be detected by
...
/**
 * ACC FI command handler.
 *
 * Command handler for the ACC FI command.
 *
 * @param uj An initialized uJSON context.
 * @return OK or error.
 */
...
#endif  // OPENTITAN_SW_DEVICE_TESTS_PENETRATIONTESTS_FIRMWARE_FI_ACC_FI_H_
