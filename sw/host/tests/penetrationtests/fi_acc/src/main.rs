...
use pentest_commands::fi_acc_commands::AccFiSubcommand;
...
struct Opts {
    ...
    #[arg(long, num_args = 1..)]
    fi_acc_json: Vec<String>,
}

#[derive(Debug, Deserialize)]
struct FiAccTestCase {
    test_case_id: usize,
    command: String,
    #[serde(default)]
    input: String,
...
fn filter_response(response: serde_json::Value) -> serde_json::Map<String, serde_json::Value> {
    ...
    // ACC instruction counter should be const.
    map.remove("insn_cnt");
    // Filter as the CharBnWsrr returns random data from ACC.
    map.remove("data");

    map
}

fn run_fi_acc_testcase(
    test_case: &FiAccTestCase,
    opts: &Opts,
    uart: &dyn Uart,
    ...
    PenetrationtestCommand::AccFi.send(uart)?;

    // Send test subcommand.
    AccFiSubcommand::from_str(test_case.command.as_str())
        .context("unsupported ACC FI subcommand")?
        .send(uart)?;
...
fn test_fi_acc(opts: &Opts, transport: &TransportWrapper) -> Result<()> {
    ...
    let test_vector_files = &opts.fi_acc_json;
    for file in test_vector_files {
        let raw_json = fs::read_to_string(file)?;
        let fi_acc_tests: Vec<FiAccTestCase> = serde_json::from_str(&raw_json)?;
        for fi_acc_test in &fi_acc_tests {
            test_counter += 1;
            log::info!("Test counter: {}", test_counter);
            run_fi_acc_testcase(fi_acc_test, opts, &*uart, &mut fail_counter)?;
        }
    }
    assert_eq!(
        0, fail_counter,
...
fn main() -> Result<()> {
    ...
    execute_test!(test_fi_acc, &opts, &transport);
    Ok(())
}
