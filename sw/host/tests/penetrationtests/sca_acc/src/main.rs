...
use pentest_commands::sca_acc_commands::AccScaSubcommand;
...
struct Opts {
    ...
    #[arg(long, num_args = 1..)]
    sca_acc_json: Vec<String>,
}

#[derive(Debug, Deserialize)]
struct ScaAccTestCase {
    test_case_id: usize,
    command: String,
    #[serde(default)]
    input: String,
...
fn run_sca_acc_testcase(
    test_case: &ScaAccTestCase,
    opts: &Opts,
    uart: &dyn Uart,
    ...
    PenetrationtestCommand::AccSca.send(uart)?;

    // Send test subcommand.
    AccScaSubcommand::from_str(test_case.command.as_str())
        .context("unsupported ACC SCA subcommand")?
        .send(uart)?;
...
fn test_sca_acc(opts: &Opts, transport: &TransportWrapper) -> Result<()> {
    ...
    let test_vector_files = &opts.sca_acc_json;
    for file in test_vector_files {
        let raw_json = fs::read_to_string(file)?;
        let sca_acc_tests: Vec<ScaAccTestCase> = serde_json::from_str(&raw_json)?;
        for sca_acc_test in &sca_acc_tests {
            test_counter += 1;
            log::info!("Test counter: {}", test_counter);
            run_sca_acc_testcase(sca_acc_test, opts, &*uart, &mut fail_counter)?;
        }
    }
    assert_eq!(
        0, fail_counter,
...
fn main() -> Result<()> {
    ...
    execute_test!(test_sca_acc, &opts, &transport);
    Ok(())
}
